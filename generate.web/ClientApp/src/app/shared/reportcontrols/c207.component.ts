...
@Component({
    selector: 'generate-app-c207',
    templateUrl: './c207.component.html',
    styleUrls: ['./c207.component.scss'],
    providers: [GenerateReportService]
})

export class C207Component implements AfterViewInit, OnChanges, OnInit {
    ...
    getAppropriationMethodDescription(appropriationMethodCode: string) {
        let description: string = '';

        if (appropriationMethodCode === 'STEAPRDRCT') {
            description = 'Direct from state';
        }
        else if (appropriationMethodCode === 'STEAPRTHRULEA') {
            description = 'Through local school district';
        }
        else if (appropriationMethodCode === 'STEAPRALLOCLEA') {
