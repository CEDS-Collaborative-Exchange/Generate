...
export class CharterSchoolComponent implements AfterViewInit, OnChanges, OnInit {
    ...
    populateReport() {
        ...
        if (this.reportParameters.reportType !== undefined
            ...
            this.repCode = this.reportParameters.reportCode;
            ...
            this._generateReportService.getReport(this.reportParameters.reportType, this.reportParameters.reportCode, this.reportParameters.reportLevel, this.reportParameters.reportYear, categorySetCode, this.reportParameters.reportSort, skip, take)
                .subscribe(
                    reportDataDto => {
                        ...
                        console.log(this.reportDataDto);
                        if (this.reportDataDto.data !== null && this.reportDataDto.data.length > 0) {
                            this.reportDataDto.data.forEach((item, index) => {
                                ...
                                item.mailingAddressStreet = this.getAddress(item.mailingAddressStreet, item.mailingAddressCity, item.mailingAddressState, item.mailingAddressPostalCode, item.mailingAddressPostalCode2);
                                item.physicalAddressStreet = this.getAddress(item.physicalAddressStreet, item.physicalAddressCity, item.physicalAddressState, item.physicalAddressPostalCode, item.physicalAddressPostalCode2);
    ...
    getAddress(addressStreet: string, addressCity: string, addressState: string, addressPostalCode: string, addressPostalCode2: string) {
        let str: string = '';
        str = this.checkAddress(addressStreet) + ' , ' + this.checkAddress(addressCity) + ' , ' + this.checkAddress(addressState) + ' , ' + this.checkAddress(addressPostalCode);
        if (str === ' ,  ,  , ') {
            str = '';
        }
        if (this.checkAddress(addressPostalCode2).length > 0) { str = str + '-' + addressPostalCode2; }
        return str;
    }

    checkAddress(add: string) {
        if (add === null)
            return '';
        return add;
    }
