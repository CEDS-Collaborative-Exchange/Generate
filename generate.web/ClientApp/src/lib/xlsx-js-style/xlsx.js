...
//        ws['!rows'][R] = { hpx: 30 };
		...
		for(_C = C = 0; _C < elts.length; ++_C) {
            ...
            if (((RS = (+elt.getAttribute("rowspan") || 1))) > 1 || CS > 1) {
                ...
                if (cssclass === "pvtRowLabel" || cssclass === "pvtColLabel") {
                    let startindex = R;
                    let endindex = R + (RS || 1);
                    for (let i = startindex + 1; i <= endindex; i++) {
                        let col = columnToLetter(C + or_C + (CS || 1));
                        let row = i;
                        ws[col + row.toString()] = mergedCell;
                    }
                }
            }
			var o = {t:'s', v:v};
            ...
            //Add styling for pivot table
            var cssclass = elt.getAttribute("class")
            if (cssclass === "pvtAxisLabel") {
                o.s = {
                    font: { bold: true, sz: 12 }
                    , fill: { fgColor: { rgb: 'cfd6d6' } }
                    , alignment: { horizontal: 'left', vertical: 'center' }
                    , border: {
                        top: { style: 'thin', color: { rgb: 'b8bebe' } }
                        , bottom: { style: 'thin', color: { rgb: 'b8bebe' } }
                        , left: { style: 'thin', color: { rgb: 'b8bebe' } }
                        , right: { style: 'thin', color: { rgb: 'b8bebe' } }
                    }
                };
                rowinfo[R] = { hpx: 30 };
            } else if (cssclass === "pvtRowLabel") {
                o.s = {
                    font: { bold: true, sz:12 }
                    , fill: { fgColor: { rgb: 'e6eeee' } }
                    , alignment: { horizontal: 'left', vertical: 'center', wrapText: true }
                ...
                console.log("cell text " + elt.innerHTML);
       //         console.log("cell text width" + rowinfo[R].hpx);
            } else if (cssclass === "pvtColLabel") {
                o.s = {
                    font: { bold: true, sz: 12 }
                    , fill: { fgColor: { rgb: 'e6eeee' } }
               ...
               // console.log("Style pvtVal:" + cssclass);
                o.s = {
                    font: { sz: 12 }
                  //  , fill: { fgColor: { rgb: 'e6eeee' } }
                ...
                if (elt.innerHTML.length <= 24) {
               //     console.log("Style pvtVal column row:" + R);
              //      rowinfo[R] = { hpx: 25 };
                }
