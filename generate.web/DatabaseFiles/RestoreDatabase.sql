...
set @databaseVersion = '3.1'
declare @databaseFilesPath as nvarchar(500)
set @databaseFilesPath = 'C:\Repos\generate_' + @databaseVersion + '.bak'
...
BEGIN
	...
	RESTORE DATABASE [' + @databaseName + '] FROM DISK = @databaseFilesPath WITH 
		NOUNLOAD, 
		REPLACE,
		MOVE ''generate-test'' TO ''' + @dataPath + '\\generate.mdf'',   
		MOVE ''generate-test_log'' TO ''' + @logPath + '\\generate_log.mdf''
