		...
		BEGIN
			SELECT
				...
				, CASE WHEN EXISTS (
					...
					FROM #Facts f
					JOIN Staging.ProgramParticipationSpecialEducation sppse
						ON f.StagingId = sppse.Id
					WHERE sppse.StudentIdentifierState = @StudentIdentifierState
				) THEN 1 ELSE 0 END AS MadeItIntoFacts
				, CASE
					WHEN ske.Id IS NULL THEN 'Staging.K12Enrollment'
					WHEN NOT EXISTS (
						SELECT 1
						FROM RDS.DimSchoolYears rsy
						WHERE ske.SchoolYear = rsy.SchoolYear
					) THEN 'JOIN RDS.DimSchoolYears'
					WHEN NOT EXISTS (
						SELECT 1
					...
					WHEN NOT EXISTS (
						SELECT 1
						FROM RDS.DimAges rda
						WHERE RDS.Get_Age(ske.Birthdate, @ChildCountDate) = rda.AgeValue
					) THEN 'JOIN RDS.DimAges'
					WHEN NOT EXISTS (
						SELECT 1
						FROM RDS.DimSeas rds
						WHERE @ChildCountDate BETWEEN rds.RecordStartDateTime AND ISNULL(rds.RecordEndDateTime, @SYEndDate)
					...
					WHEN NOT EXISTS (
						...
						WHERE ske.SchoolYear = sppse.SchoolYear
							...
							AND ISNULL(ske.SchoolIdentifierSea,'') = ISNULL(sppse.SchoolIdentifierSea,'')
							AND @ChildCountDate BETWEEN sppse.ProgramParticipationStartDate AND ISNULL(sppse.ProgramParticipationExitDate, @SYEndDate)
					) THEN 'JOIN Staging.ProgramParticipationSpecialEducation'
					WHEN ske.EnrollmentEntryDate IS NULL OR @ChildCountDate NOT BETWEEN ske.EnrollmentEntryDate AND ISNULL(ske.EnrollmentExitDate, @SYEndDate) THEN 'WHERE EnrollmentEntryDate/EnrollmentExitDate'
					WHEN NOT EXISTS (
						SELECT 1
						FROM #Facts f
						JOIN Staging.ProgramParticipationSpecialEducation sppse
							ON f.StagingId = sppse.Id
						WHERE sppse.StudentIdentifierState = @StudentIdentifierState
					) THEN 'Unknown - all required joins matched but no #Facts row was inserted'
					ELSE 'Inserted into #Facts'
				END AS DebugResult
				, (SELECT COUNT(1) FROM #Facts) AS FactsRowsForStudent
				, CASE WHEN ske.EnrollmentEntryDate IS NOT NULL AND @ChildCountDate BETWEEN ske.EnrollmentEntryDate AND ISNULL(ske.EnrollmentExitDate, @SYEndDate) THEN 1 ELSE 0 END AS PassedEnrollmentDateFilter
				...
				, CASE WHEN EXISTS (SELECT 1 FROM RDS.DimAges rda WHERE RDS.Get_Age(ske.Birthdate, @ChildCountDate) = rda.AgeValue) THEN 1 ELSE 0 END AS MatchedDimAges
				, CASE WHEN EXISTS (SELECT 1 FROM RDS.DimSeas rds WHERE @ChildCountDate BETWEEN rds.RecordStartDateTime AND ISNULL(rds.RecordEndDateTime, @SYEndDate)) THEN 1 ELSE 0 END AS MatchedDimSeas
				, CASE WHEN EXISTS (
					SELECT 1
					FROM Staging.ProgramParticipationSpecialEducation sppse
					WHERE ske.SchoolYear = sppse.SchoolYear
						...
						AND ISNULL(ske.SchoolIdentifierSea,'') = ISNULL(sppse.SchoolIdentifierSea,'')
						AND @ChildCountDate BETWEEN sppse.ProgramParticipationStartDate AND ISNULL(sppse.ProgramParticipationExitDate, @SYEndDate)
				) THEN 1 ELSE 0 END AS MatchedProgramParticipationSpecialEducation
