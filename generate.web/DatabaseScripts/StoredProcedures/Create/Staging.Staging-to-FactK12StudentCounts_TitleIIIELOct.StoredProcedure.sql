			...
			SELECT
				...
				, CASE
					...
					WHEN NOT EXISTS (
						...
						WHERE @ReportingDate BETWEEN CONVERT(DATE, rds.RecordStartDateTime) AND ISNULL(CONVERT(DATE, rds.RecordEndDateTime), @SYEndDate)
					...
					WHEN ske.EnrollmentEntryDate IS NULL OR @ReportingDate NOT BETWEEN ske.EnrollmentEntryDate AND ISNULL(ske.EnrollmentExitDate, @SYEndDate) THEN 'WHERE EnrollmentEntryDate/EnrollmentExitDate'
					WHEN NOT EXISTS (
						SELECT 1
						FROM #vwGradeLevels rgls
						WHERE ske.GradeLevel = rgls.GradeLevelMap
							AND rgls.GradeLevelTypeDescription = 'Entry Grade Level'
							AND rgls.GradeLevelCode IN ('KG', '01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12', '13', 'UG')
					) THEN 'WHERE GradeLevelCode'
				...
				, CASE WHEN EXISTS (SELECT 1 FROM RDS.DimSeas rds WHERE @ReportingDate BETWEEN CONVERT(DATE, rds.RecordStartDateTime) AND ISNULL(CONVERT(DATE, rds.RecordEndDateTime), @SYEndDate)) THEN 1 ELSE 0 END AS MatchedDimSeas
				, CASE WHEN EXISTS (
					SELECT 1
					FROM Staging.PersonStatus el
					WHERE ske.SchoolYear = el.SchoolYear
				...
				, CASE WHEN ske.EnrollmentEntryDate IS NOT NULL AND @ReportingDate BETWEEN ske.EnrollmentEntryDate AND ISNULL(ske.EnrollmentExitDate, @SYEndDate) THEN 1 ELSE 0 END AS PassedEnrollmentDateFilter
				, CASE WHEN EXISTS (
					SELECT 1
					FROM #vwGradeLevels rgls
					WHERE ske.GradeLevel = rgls.GradeLevelMap
