...
AS 
	...
	LEFT JOIN app.GenerateReport_OrganizationLevels grol
		...
		AND cs.OrganizationLevelId IS NULL   -- 14.1: only fall back to the report org-level list when the set pins no level (else grol fans each set across every level -> duplicate pivot aliases)
	JOIN app.OrganizationLevels aol
		-- 14.1: key each category set to its OWN org level (fall back to the report's org-level
		-- list only when the set has none). The prior `cs.OrganizationLevelId = aol... OR
		-- grol... = aol...` fanned every set across every report level, so a report with the same
		-- CategorySetCode at multiple levels (e.g. FS194 CSA at SEA+LEA) emitted duplicate pivot
		-- aliases (Msg 1011 "correlation name 'pv<CAT>' specified multiple times"). This also
		-- matches how Staging.RunEndToEndTest keys the expected side (strictly cs.OrganizationLevelId).
		ON aol.OrganizationLevelId = ISNULL(cs.OrganizationLevelId, grol.OrganizationLevelId)
	LEFT JOIN app.GenerateReport_TableType grtt
		ON gr.GenerateReportId = grtt.GenerateReportId
		AND cs.TableTypeId IS NULL   -- 14.1: only fall back to the report table-type list when the set pins no table type (else grtt fans each set -> duplicate pivot aliases)
	JOIN app.TableTypes att
		ON att.TableTypeId = ISNULL(cs.TableTypeId, grtt.TableTypeId)
