...
----Join to get the child count date, uses the Day/Month and gets the year from the last migration that was run
	JOIN Staging.K12Enrollment								enrollment
		...
		-- 14.1: keep the enrollment window but default a null exit to the fiscal-year END (not GETDATE(),
		-- which precedes the ~Oct child-count date and dropped null-exit enrollments -> 8 of 9 fact students lost).
		AND toggle.ChildCountDate BETWEEN enrollment.EnrollmentEntryDate AND ISNULL(enrollment.EnrollmentExitDate, staging.GetFiscalYearEndDate(toggle.SchoolYear))
	JOIN Staging.ProgramParticipationSpecialEducation	programparticipation
			...
			AND		toggle.ChildCountDate BETWEEN programparticipation.ProgramParticipationStartDate AND ISNULL(programparticipation.ProgramParticipationExitDate, staging.GetFiscalYearEndDate(toggle.SchoolYear))
			--AND		programparticipation.ProgramParticipationBeginDate  BETWEEN enrollment.EnrollmentEntryDate AND ISNULL(enrollment.EnrollmentExitDate, staging.GetFiscalYearEndDate(toggle.SchoolYear))
	...
	LEFT JOIN Staging.IdeaDisabilityType					ideaDisability
			...
			AND		Toggle.ChildCountDate BETWEEN ideaDisability.RecordStartDateTime AND ISNULL(ideaDisability.RecordEndDateTime, staging.GetFiscalYearEndDate(toggle.SchoolYear))
			--AND		ideaDisability.RecordStartDateTime  BETWEEN enrollment.EnrollmentEntryDate AND ISNULL(enrollment.EnrollmentExitDate, staging.GetFiscalYearEndDate(toggle.SchoolYear))
	...
	LEFT JOIN Staging.K12PersonRace							race
			...
			AND		Toggle.ChildCountDate BETWEEN race.RecordStartDateTime AND ISNULL(race.RecordEndDateTime, staging.GetFiscalYearEndDate(toggle.SchoolYear))
			--AND		race.RecordStartDateTime  BETWEEN enrollment.EnrollmentEntryDate AND ISNULL(enrollment.EnrollmentExitDate, staging.GetFiscalYearEndDate(toggle.SchoolYear))
	...
	LEFT JOIN Staging.PersonStatus							el
			...
			AND		Toggle.ChildCountDate BETWEEN el.EnglishLearner_StatusStartDate AND ISNULL(el.EnglishLearner_StatusExitDate, staging.GetFiscalYearEndDate(toggle.SchoolYear))
			--AND		el.EnglishLearner_StatusStartDate  BETWEEN enrollment.EnrollmentEntryDate AND ISNULL(enrollment.EnrollmentExitDate, staging.GetFiscalYearEndDate(toggle.SchoolYear))
