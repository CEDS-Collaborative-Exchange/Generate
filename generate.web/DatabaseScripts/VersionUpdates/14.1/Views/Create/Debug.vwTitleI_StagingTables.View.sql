...
AS
	...
	JOIN Staging.ProgramParticipationTitleI					titleI
		...
		-- 14.1: match the TitleI fact proc's participation window (overlap of the enrollment span)
	...
	WHERE 1 = 1
	-- 14.1: removed the title-I-SCHOOL-status filter (sssrd.OutputCode IN 'TGELGBTGPROG'/'SWELIGTGPROG'/
	-- 'SWELIGSWPROG') — the TitleI fact proc does not restrict by school title-I status, so this made the
	-- base far smaller than the fact population (516 vs 1556).
