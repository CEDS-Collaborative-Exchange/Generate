...
AS
	SELECT	DISTINCT 
		...
		, idea.IDEAIndicator
		, idea.IDEAEducationalEnvironmentForEarlyChildhood
		, idea.IDEAEducationalEnvironmentForSchoolAge
	...
	INNER JOIN Staging.PersonStatus							hm
			...
			AND		hm.Homelessness_StatusStartDate  BETWEEN enrollment.EnrollmentEntryDate AND ISNULL(enrollment.EnrollmentExitDate, staging.GetFiscalYearEndDate(enrollment.SchoolYear))
	...
	LEFT JOIN Staging.PersonStatus							hmnr
			...
			AND		hmnr.HomelessNightTimeResidence_StartDate  BETWEEN enrollment.EnrollmentEntryDate AND ISNULL(enrollment.EnrollmentExitDate, staging.GetFiscalYearEndDate(enrollment.SchoolYear))
	...
	LEFT JOIN Staging.K12PersonRace							race
			...
			AND		race.RecordStartDateTime  BETWEEN enrollment.EnrollmentEntryDate AND ISNULL(enrollment.EnrollmentExitDate, staging.GetFiscalYearEndDate(enrollment.SchoolYear))
