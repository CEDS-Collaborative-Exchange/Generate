...
BEGIN
	...
	--Create the temp tables (and any relevant indexes) needed for this domain
		...
		WHERE SchoolYear = @SchoolYear
		AND isnull(Iso6392LanguageMap, '') <> ''  -- 14.1: exclude unmapped languages. Without this, an unmapped/NULL source native language ('MISSING' fallback) matched EVERY language row whose map is NULL -> CSB language cartesian (FS141 summed 14,503 vs a 315 total). Mirrors the FS116 SY sibling proc (its line ~125).
