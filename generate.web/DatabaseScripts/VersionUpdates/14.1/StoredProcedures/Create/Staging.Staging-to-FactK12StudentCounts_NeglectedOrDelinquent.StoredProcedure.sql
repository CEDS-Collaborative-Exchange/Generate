...
BEGIN
	...
	--Create and load #Facts temp table
		...
		SELECT DISTINCT
			...
			, ISNULL(rds.Get_DimDate(sppnord.ProgramParticipationExitDate), -1)		StatusEndDateNeglectedOrDelinquentId
	...
	--idea disability (RDS)
		LEFT JOIN RDS.vwDimIdeaStatuses rdis
			...
			AND ISNULL(TRY_CAST(idea.IDEAIndicator AS SMALLINT), -1) = ISNULL(rdis.IdeaIndicatorMap, -1)
	...
	--english learner (RDS)
		LEFT JOIN RDS.vwDimEnglishLearnerStatuses rdels
			...
			AND ISNULL(TRY_CAST(el.EnglishLearnerStatus AS SMALLINT), -1) = ISNULL(rdels.EnglishLearnerStatusMap, -1)
