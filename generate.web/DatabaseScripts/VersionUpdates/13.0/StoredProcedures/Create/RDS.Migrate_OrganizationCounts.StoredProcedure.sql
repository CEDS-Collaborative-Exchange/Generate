...
BEGIN TRY
	...
	BEGIN
		...
		SELECT DimLeaId, LeaIdentifierState 
		FROM rds.DimLeas 
		WHERE DimLeaId <> -1 
		AND ReportedFederally = 1
		AND ((RecordStartDateTime < staging.GetFiscalYearStartDate(@SchoolYear) and RecordEndDateTime IS NULL)
			OR (RecordStartDateTime >= staging.GetFiscalYearStartDate(@SchoolYear) and RecordStartDateTime <= staging.GetFiscalYearEndDate(@SchoolYear)))
		...
		SELECT DimK12SchoolId, SchoolIdentifierState 
		FROM rds.DimK12Schools 
		WHERE DimK12SchoolId <> -1
		AND ReportedFederally = 1
		AND ((RecordStartDateTime < staging.GetFiscalYearStartDate(@SchoolYear) and RecordEndDateTime IS NULL)
			OR (RecordStartDateTime >= staging.GetFiscalYearStartDate(@SchoolYear) and RecordStartDateTime <= staging.GetFiscalYearEndDate(@SchoolYear)))
