...
inner join DataItem di on defsd.DataElementID = di.DataElementID
left outer join DataItem_x_Code dicode on dicode.DataItemID = di.DataItemID
inner join Code c on dicode.CodeID = c.CodeID
...
BEGIN
	...
	inner join DataItem di on defsd.DataElementID = di.DataElementID
	left outer join DataItem_x_Code dicode on dicode.DataItemID = di.DataItemID
	inner join Code c on c.CodeId = dicode.CodeId
	where fsd.FileSpecificationDocumentNumber like 'N206%'
...
begin try
	...
	begin transaction
		...
		VALUES
		(@GenerateReportId, @schId, 0, '2019-20', 'CategorySetA', 'Category Set A')
