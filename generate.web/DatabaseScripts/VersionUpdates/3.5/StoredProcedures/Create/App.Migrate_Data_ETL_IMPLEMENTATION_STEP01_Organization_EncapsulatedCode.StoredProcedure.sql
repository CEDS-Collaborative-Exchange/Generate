...
BEGIN
	...
	BEGIN TRY
		IF (SELECT COUNT(*) FROM ODS.OrganizationDetail od join Staging.StateDetail sd on od.Name = sd.SeaName) < 1 BEGIN
			...
			SELECT DISTINCT
						...
						,[App].[GetOrganizationIdentifierSystemId]('State', '001491') [RefOrganizationIdentificationSystemId] --Federal identification from the RefOrganizationIdentificationSystem table--
						,sd.OrganizationId [OrganizationId]
						,[App].[GetOrganizationIdentifierTypeId]('001491') [RefOrganizationIdentifierTypeId] --State Agency Identification System from the RefOrganizationIdentifierType table --
						,sd.RecordStartDateTime
						,sd.RecordEndDateTime
	...
	*/
	...
	BEGIN TRY
		...
		AND ooi.RefOrganizationIdentifierTypeId = [App].[GetOrganizationIdentifierTypeId]('001072')
		AND ooi.RefOrganizationIdentificationSystemId = [App].[GetOrganizationIdentifierSystemId]('SEA', '001072')
...
---------------------------------
	...
	BEGIN TRY
		...
		WHERE orgid.RefOrganizationIdentificationSystemId = [App].[GetOrganizationIdentifierSystemId]('SEA', '001072')
	...
	BEGIN TRY
		...
		WHERE orgid.RefOrganizationIdentificationSystemId = [App].[GetOrganizationIdentifierSystemId]('NCES', '001072')
			AND orgid.RefOrganizationIdentifierTypeId = [App].[GetOrganizationIdentifierTypeId]('001072')
			AND org.LEA_RecordStartDateTime IS NOT NULL
	...
	BEGIN TRY
		...
		SELECT DISTINCT 
			...
			, App.GetOrganizationIdentifierSystemId('SEA', '001072')
	...
	BEGIN TRY
		...
		WHEN NOT MATCHED BY TARGET THEN 
			...
			VALUES (
				...
				, [App].[GetOrganizationIdentifierSystemId]('SEA', '001072')
				, SOURCE.OrganizationId
				, [App].[GetOrganizationIdentifierTypeId]('001072')
				, SOURCE.RecordStartDateTime
				, SOURCE.RecordEndDateTime);
	END TRY
	...
	BEGIN TRY
		...
		SELECT DISTINCT 
			...
			, org.LEAOrganizationId
			, App.GetOrganizationIdentifierSystemId('NCES', '001072')
			, org.LEA_RecordStartDateTime
			, org.LEA_RecordEndDateTime
		FROM Staging.Organization org
	...
	BEGIN TRY
		...
		WHERE orgid.RefOrganizationIdentificationSystemId = [App].[GetOrganizationIdentifierSystemId]('SEA', '001073')
			AND orgid.RefOrganizationIdentifierTypeId = [App].[GetOrganizationIdentifierTypeId]('001073')
			AND org.School_RecordStartDateTime IS NOT NULL
	...
	BEGIN TRY
		...
		WHERE orgid.RefOrganizationIdentificationSystemId = [App].[GetOrganizationIdentifierSystemId]('NCES', '001073')
			AND orgid.RefOrganizationIdentifierTypeId = [App].[GetOrganizationIdentifierTypeId]('001073')
			AND org.School_RecordStartDateTime IS NOT NULL
	...
	BEGIN TRY
		...
		WHERE orgid.RefOrganizationIdentificationSystemId = [App].[GetOrganizationIdentifierSystemId]('Federal', '001073')
			AND orgid.RefOrganizationIdentifierTypeId = [App].[GetOrganizationIdentifierTypeId]('001073')
			AND org.School_RecordStartDateTime IS NOT NULL
	...
	BEGIN TRY
		...
		SELECT DISTINCT 
			...
			, org.SchoolOrganizationId
			, App.GetOrganizationIdentifierSystemId('SEA', '001073')
			, org.School_RecordStartDateTime
			, org.School_RecordEndDateTime
		FROM Staging.Organization org
	...
	BEGIN TRY
		...
		WHEN NOT MATCHED BY TARGET THEN 
			...
			VALUES (
				...
				, [App].[GetOrganizationIdentifierSystemId]('SEA', '001073')
				, SOURCE.OrganizationId
				, [App].[GetOrganizationIdentifierTypeId]('001073')
				, SOURCE.RecordStartDateTime
				, SOURCE.RecordEndDateTime);
	...
	BEGIN TRY
		...
		SELECT DISTINCT 
			...
			, org.SchoolOrganizationId
			, App.GetOrganizationIdentifierSystemId('NCES', '001073')
			, org.School_RecordStartDateTime
			, org.School_RecordEndDateTime
		FROM Staging.Organization org
	...
	BEGIN TRY
		...
		SELECT DISTINCT 
			...
			, org.SchoolOrganizationId
			, App.GetOrganizationIdentifierSystemId('Federal', '001073')
			, org.School_RecordStartDateTime
			, org.School_RecordEndDateTime
		FROM Staging.Organization org
	...
	BEGIN TRY
		...
		WHERE ot.RefOrganizationTypeId = App.GetOrganizationTypeId('SEA', '001156')
			AND orgid.RefOrganizationIdentificationSystemId = [App].[GetOrganizationIdentifierSystemId]('Federal', '001491')
	...
	BEGIN TRY
        ...
        UPDATE ooos 
		SET ooos.RecordEndDateTime = s.RecordStartDateTime - 1
        FROM ODS.OrganizationOperationalStatus ooos
        JOIN (
				SELECT 
					OrganizationId
					, MAX(OrganizationOperationalStatusId) AS OrganizationOperationalStatusId
					, MAX(RecordStartDateTime) AS RecordStartDateTime
				FROM ODS.OrganizationOperationalStatus
				WHERE RecordEndDateTime IS NULL
					AND RecordStartDateTime IS NOT NULL
				GROUP BY OrganizationId, OrganizationOperationalStatusId, RecordStartDateTime
        ) s ON ooos.OrganizationId = s.OrganizationId
                AND ooos.OrganizationOperationalStatusId <> s.OrganizationOperationalStatusId 
                AND ooos.RecordEndDateTime IS NULL
				AND ooos.RecordStartDateTime IS NOT NULL
				AND ooos.RecordStartDateTime < s.RecordStartDateTime
...
---first check to see if it exists first so it's not created again? Need to add in the merge process and Record Start End Dates process for all of this
	...
	BEGIN TRY
		...
		JOIN ods.OrganizationIdentifier oi
			...
			AND oi.RefOrganizationIdentificationSystemId = [App].[GetOrganizationIdentifierSystemId]('SEA', '001072')
			AND oi.RefOrganizationIdentifierTypeId = [App].[GetOrganizationIdentifierTypeId]('001072')
	END TRY
