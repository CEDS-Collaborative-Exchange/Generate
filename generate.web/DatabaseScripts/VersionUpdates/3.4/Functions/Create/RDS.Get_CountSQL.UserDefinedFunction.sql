...
BEGIN
	  ...
	  begin
		if(@includeOrganizations = 1)
		  begin
			...
			+ ' where ' + case when @reportLevel = 'lea' then 's.DimLeaId <> -1 
	...
	BEGIN
		...
		if @sqlType = 'actual'
			begin
				...
				else if @categoryCode = 'PROFSTATUS' and @reportCode='yeartoyearprogress'
					begin
						set @sqlCategoryReturnField = ' 
							case 
			...
			else if (@reportField = 'PROFICIENCYSTATUS' and @reportCode in ('yeartoyearprogress'))
			BEGIN
					set @sqlCountJoins = @sqlCountJoins + '		
		...
		begin
			if(@reportCode = 'c006')
				begin
					...
					from ( select ' + case when @reportLevel = 'lea' then 'fact.DimLeaId,' 
									when @reportLevel = 'sch' then 'fact.DimSchoolId,'
									else ''
	...
	begin
		...
		begin
			...
			inner join ( select ' + @sqlCategoryFields + 
			' from @reportData
