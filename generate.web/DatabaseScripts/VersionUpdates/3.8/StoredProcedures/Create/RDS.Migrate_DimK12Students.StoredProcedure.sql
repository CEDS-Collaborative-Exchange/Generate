	...
	BEGIN TRY
       ...
       , CTE as 
		...
		WHEN MATCHED AND (
				...
				OR ISNULL(trgt.Cohort, '') <> ISNULL(src.Cohort, '')
				OR ISNULL(trgt.SexCode, '') <> ISNULL(src.SexCode, '')
				)
				THEN 
			UPDATE SET 
				...
				, trgt.SexCode = src.SexCode
				, trgt.SexDescription = src.SexDescription
				, trgt.SexEdFactsCode = src.SexEdFactscode
