...
BEGIN
			...
			begin
				...
				begin
					...
					begin
						set @sqlCategoryReturnField = ' 
							case 
								when CAT_' + @reportField + '.HomelessnessStatusCode = ''Homeless'' then ''H''					
								else CAT_' + @reportField + '.' + @dimensionField + '
							end'
					end
	...
	begin
		...
		set @sqlCountJoins = @sqlCountJoins + '
			...
			and fact.K12StudentId NOT IN (SELECT K12StudentId FROM RDS.FactK12StudentDisciplines sd GROUP BY K12StudentId HAVING SUM(sd.DisciplineDuration) > 45)'
		...
		set @sqlCountJoins = @sqlCountJoins + '
			inner join (
				...
				where idea.IdeaEducationalEnvironmentCode <> ''PPPS''
					and idea.IdeaIndicatorCode = ''IDEA'' and d.IdeaInterimRemovalCode = ''REMDW''
	...
	END
		...
		set @sqlCountJoins = @sqlCountJoins + '
			inner join (
				...
				where demo.HomelessnessStatusCode=''Homeless''
