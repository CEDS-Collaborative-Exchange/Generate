...
begin transaction	
	...
	*/
		...
		DECLARE @SeaSourceData as table (
			...
			, MailingStreet2 nvarchar(40) NULL
			...
			, PhysicalStreet2 nvarchar(40) NULL
		...
		select 
			...
			mailingAddress.ApartmentRoomOrSuiteNumber as MailingStreet2,
			...
			physicalAddress.ApartmentRoomOrSuiteNumber as PhysicalStreet2,
		...
		left join (
					select ol.OrganizationId, ol.LocationId, RefOrganizationLocationTypeId, StreetNumberAndName, ApartmentRoomOrSuiteNumber,City, 
		...
		WHEN MATCHED THEN 
				UPDATE SET 
				 ...
				 trgt.MailingAddressStreet2 = src.MailingStreet2,
				 ...
				 trgt.PhysicalAddressStreet2 = src.PhysicalStreet2,
		...
		(
			...
			StateANSICode,
			StateCode,
			StateName,MailingAddressStreet,
	...
	*/
		...
		SELECT DISTINCT	
			o.Name, isnull(i.Identifier, '') as NCESIdentifier, 
		...
		left join  (select ol.OrganizationId, ol.LocationId, RefOrganizationLocationTypeId, StreetNumberAndName, ApartmentRoomOrSuiteNumber,City, 
					refState.Code as StateCode, PostalCode 
					from ODS.OrganizationLocation ol
						inner join ods.LocationAddress la 
							on ol.LocationId = la.LocationId
	...
	*/
		...
		SELECT DISTINCT  
			...
			leaStateIdentifier.Identifier AS LeaStateIdentifier,
			...
			priorSchool.Identifier as PriorSchoolStateIdentifier, 
			K12.CharterSchoolIndicator, k12.CharterSchoolContractApprovalDate,	
			...
			mailingAddress.ApartmentRoomOrSuiteNumber,  
			mailingAddress.City AS MailingCity, 
			mailingAddress.StateCode AS MailingState, 
			mailingAddress.PostalCode AS MailingPostalCode, 
			CASE WHEN physicalAddress.StateCode <> @stateCode or mailingAddress.StateCode <> @stateCode THEN '1' ELSE 0 END AS OutOfState,
		...
		left outer join ods.OrganizationOperationalStatus op 
			ON o.OrganizationId = op.OrganizationId
            AND startdate.RecordStartDateTime between op.RecordStartDateTime and isnull(op.RecordEndDateTime, getdate())
		left outer join ods.RefOperationalStatus refop 
			on refop.RefOperationalStatusId = op.RefOperationalStatusId 
		...
		(
			...
			src.LeaStateIdentifier, 
			src.LeaOrganizationName, 
			src.SchoolNcesIdentifier, 
			src.SchoolStateIdentifier, 
			src.SchoolName, 
			src.SchOperationalStatus, 
			src.SchOperationalEdfactsStatus, 
			src.OperationalStatusEffectiveDate,  
			src.PriorLeaStateIdentifier,
			src.PriorSchoolStateIdentifier, 
