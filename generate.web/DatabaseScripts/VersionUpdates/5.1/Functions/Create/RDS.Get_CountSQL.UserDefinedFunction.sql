...
BEGIN
			...
			if @reportCode in ('c088', 'c143')
				begin
					...
					select @sql = @sql + 
						...
						GROUP BY K12StudentId HAVING SUM(sd.DisciplineDuration) >= 0.5' + char(10)
					end
					...
					begin
						...
						GROUP BY K12StudentId, d.IdeaInterimRemovalCode, d.IdeaInterimRemovalReasonCode  HAVING SUM(sd.DisciplineDuration) > 45' + char(10)
					...
					begin
						...
						' SELECT K12StudentId into #Students 
						 FROM RDS.FactK12StudentDisciplines sd 
						INNER JOIN rds.DimDisciplines dd ON sd.DisciplineId = dd.DimDisciplineId
						INNER JOIN rds.DimIdeaStatuses id ON sd.IdeaStatusId = id.DimIdeaStatusId
						WHERE dd.IdeaInterimRemovalEdFactsCode in (''REMDW'', ''REMHO'')
						AND id.IdeaIndicatorEdFactsCode = ''IDEA''
						GROUP BY K12StudentId HAVING SUM(sd.DisciplineDuration) > 45' + char(10)
					end
	...
	begin
		...
		set @sqlCountJoins = @sqlCountJoins + '
			inner join (
				...
				where idea.IdeaEducationalEnvironmentCode <> ''PPPS''
					...
					and fact.K12StudentId NOT IN (SELECT K12StudentId FROM #Students)'
		...
		set @sqlCountJoins = @sqlCountJoins + '
			...
			-- JW 7/20/2022
				...
				'and fact.K12StudentId NOT IN (select K12StudentId from #Students)' + char(10)
		...
		-- Ages 3-21, Has Disability, Duration >= 0.5 
			...
			select @sqlCountJoins = @sqlCountJoins + '
				INNER JOIN #Students Students
					on Students.K12StudentId = fact.K12StudentId' + char(10)
	...
	if(@factReportTable = 'ReportEDFactsK12StaffCounts')
		...
		begin
			...
			else if(@reportCode in ('c005','c086','c144','c007', 'disciplinaryremovals'))
				begin
					set @sql = @sql + '
	...
	***************************************/
		...
		IF @reportCode NOT IN ('C070', 'C099', 'C112') BEGIN
		...
		IF @reportCode NOT IN ('C070', 'C099', 'C112') BEGIN
			set @debugTableCreate += '		order by s.StateStudentIdentifier ' + char(10)
		END ELSE BEGIN
