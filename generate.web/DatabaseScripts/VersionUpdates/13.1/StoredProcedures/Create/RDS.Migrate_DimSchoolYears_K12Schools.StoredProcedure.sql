CREATE PROCEDURE [RDS].[Migrate_DimSchoolYears_K12Schools]
	@dimSchoolId as int
AS
BEGIN
	...
	if @customChildCountDate is not null
	begin
		...
		begin
			...
			select @childCountDay = SUBSTRING(@customChildCountDate, CHARINDEX('/', @customChildCountDate) + 1, 2)
		end			
	end
	...
	select distinct
		@dimSchoolId,
		d.DimSchoolYearId,
		DATEFROMPARTS(d.SchoolYear - 1, @childCountMonth, @childCountDay) as DateValue,
		d.SchoolYear,
		DATEFROMPARTS(d.SchoolYear - 1, @referencePeriodStartMonth, @referencePeriodStartDay) as StartDate,
		DATEFROMPARTS(d.SchoolYear, @referencePeriodEndMonth, @referencePeriodEndDay) as EndDate
