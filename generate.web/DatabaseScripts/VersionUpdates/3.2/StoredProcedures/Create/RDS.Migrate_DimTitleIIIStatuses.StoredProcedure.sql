...
BEGIN
	...
	FROM ods.OrganizationPersonRole r1
		inner join ods.OrganizationRelationship op on r1.OrganizationId = op.OrganizationId  and r1.RoleId = @k12StudentRoleId
	...
	select PersonId as PersonId, count(*)-1 as NumberOfYear, fy.OrganizationId  
	from
	(select distinct r.PersonId, 1 as NumberOfYear, op.Parent_OrganizationId as OrganizationId
	from ods.OrganizationPersonRole r
	inner join ods.OrganizationRelationship op on r.OrganizationId = op.OrganizationId
...
FROM  @studentDates s  
	...
	left outer join (select distinct PersonId, SchoolOrganizationId, ind.Code as AssessmentParticipationCode
