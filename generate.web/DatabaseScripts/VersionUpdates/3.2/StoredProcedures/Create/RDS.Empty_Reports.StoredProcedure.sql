...
begin try
	...
	from rds.DimFactTypes where factTypeCode = @factTypeCode
		...
		begin	
				if(@reportType='organizationcounts')
				begin
				...
				else if(@reportType='studentcounts')
				begin
					delete from rds.FactStudentCountReports where ReportCode in (
					select ReportCode
					from app.GenerateReports r
					where r.ReportCode in ('studentcount','studentsex','studentdisability','studentrace','studentswdtitle1','studentsubpopulation') and r.IsLocked=1) 
					and ReportYear in (
					select distinct SubmissionYear	from rds.DimDates d		
					inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
					inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
		...
		begin	
				delete from rds.FactStudentCountReports where ReportCode in (
					select ReportCode
					from app.GenerateReports r
					where r.ReportCode in ('c002', 'c089', 'yeartoyearenvironmentcount', 'yeartoyearchildcount', 'studentssummary') and r.IsLocked=1) 
					and  ReportYear in (
		...
		begin
				...
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c009','exitspecialeducation','yeartoyearexitcount') and r.IsLocked=1)  and  ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 	
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c082','c083','c132','c154','c155','c156','c158','c169') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c033','c052') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c032') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c040') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('indicator9','indicator10') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c116','c141') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c045','c204') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c037','c134') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c054','c121','c122','c145') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c165') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c119','c127','c180','c181') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c118','c194') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c195') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c150','c151') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
			...
			delete from rds.FactStudentCountReports where ReportCode in (
				select ReportCode
				from app.GenerateReports r
				where r.ReportCode in ('c160') and r.IsLocked=1) and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
				and dm.DataMigrationTypeCode = 'report') 
		...
		begin	
				...
				from app.GenerateReports r
				inner join app.GenerateReportTypes t on r.GenerateReportTypeId = t.GenerateReportTypeId
				and t.ReportTypeCode in ('statereport') and r.IsLocked=1)			
				and ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
				inner join app.DataMigrationTypes dm on dm.DataMigrationTypeId=dd.DataMigrationTypeId 
		...
		begin
			...
			begin
				...
				from app.GenerateReports r
				inner join app.GenerateReportTypes t on r.GenerateReportTypeId = t.GenerateReportTypeId
				and r.ReportCode in ('c005', 'c006', 'c007','c086','c088','c143','c144','disciplinaryremovals','yeartoyearremoval') and r.IsLocked=1) and  ReportYear in (
				select SubmissionYear	from rds.DimDates d		
				inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId and dd.IsSelected=1
