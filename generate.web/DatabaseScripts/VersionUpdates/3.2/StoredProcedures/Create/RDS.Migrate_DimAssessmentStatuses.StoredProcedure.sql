...
BEGIN
	...
	SELECT reg.PersonId, org.DimSchoolId, org.DimLeaId, org.DimCountDateId,  min(adm.StartDate) as 'FirstAssessedDate'
        from  #studentOrganizations org
            inner join ods.AssessmentRegistration reg on org.PersonId = reg.PersonId 
            ...
            inner join (select RefAcademicSubjectId from ods.RefAcademicSubject where Code = '00256') sub on ass.RefAcademicSubjectId = sub.RefAcademicSubjectId
	 ...
	 insert into #progressLevel(PersonId, DimSchoolId, DimLeaId, DimCountDateId, AcademicSubject, ProgressLevel)            
		SELECT reg.PersonId, org.DimSchoolId, org.DimLeaId, org.DimCountDateId, sub.Code as AcademicSubject, levels.Code as ProgressLevel
			from @studentDates s
				...
				inner join ods.PersonProgramParticipation ppp on r.OrganizationPersonRoleId = ppp.OrganizationPersonRoleId 
				...
				inner join ods.OrganizationCalendar oc on oc.OrganizationId = r.OrganizationId
				inner join ods.OrganizationCalendarSession ocs on oc.OrganizationCalendarId = ocs.OrganizationCalendarId and adm.StartDate between ocs.BeginDate and ocs.EndDate



	
		SELECT 		s.DimStudentId,	IIF(aft.DimSchoolId > 0, aft.DimSchoolId, progress.DimSchoolId) as DimSchoolId,		
		...
		FROM  @studentDates s 
		left outer join #firstAssessed aft on aft.PersonId = s.PersonId   
		left outer join #progressLevel progress on progress.PersonId = s.PersonId
