...
BEGIN
	...
	and upper(d.DimensionFieldName) <> 'YEAR' and (d.DimensionFieldName) <>'AGE'
	...
	SET @ParmDefinition = N'@reportCode varchar(100), @reportYear varchar(100), @reportLevel varchar(100)';  
	...
	SET @ParDef = N'@reportCode varchar(50), @reportYear varchar(50), @reportLevel varchar(50)';  
	...
	set  @sqlrun='		SELECT CAST(ROW_NUMBER() OVER(ORDER BY Category ASC) AS INT) as FactCustomCountId,
					...
					Case when CategorySetCode=''earlychildhood'' then ''Early Childhood (3-5)''
					else ''School Age (6-21)'' end as CategorySetCode,
					...
					OrganizationName,
					ParentOrganizationStateId,
					Concat (options.CategoryOptionName,'' ('', options.CategoryOptionCode,'')'') as Category,
					[YEAR],
					StudentCount,
					CategorySetCode
					FROM #reportCounts r
					left join (SELECT distinct co.CategoryOptionCode, co.CategoryOptionName FROM 
						...
						and d.DimensionFieldName<>''AGE'') options
					...
					--PRINT @sqlRun
					EXECUTE sp_executesql @sqlRun, @ParDef, @reportCode = @reportCode, @reportYear=@reportYear, @reportLevel=@reportLevel;
