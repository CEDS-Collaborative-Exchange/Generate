...
BEGIN
	...
	declare @cohortYear varchar(10), @cohortYearTotal varchar(10), @dimFactTypeId as int
	  ...
	  begin
		if(@includeOrganizations = 1)
		  begin
			...
			inner join ' 
			+ case when @reportLevel = 'lea' then 'rds.DimLeas'
				   else 'rds.DimSchools' end  
			+ ' s on ' 
			+ case when @reportLevel = 'lea' then 'fact.DimLeaId = s.DimLeaId'
				   else 'fact.DimSchoolId = s.DimSchoolId' end  
			+ ' 
			inner join (select distinct ' + case when @reportLevel = 'lea' then 'DimLeaId' else 'DimSchoolId' end + '
						from rds.FactOrganizationCounts o
						inner join rds.DimDirectoryStatuses ds on o.DimDirectoryStatusId = ds.DimDirectoryStatusId
						where o.DimCountDateId = @dimDateId and ds.OperationalStatusEdFactsCode not in (''Closed'', ''Future'', ''Inactive'', ''MISSING'')) org	
			on ' 
			+ case when @reportLevel = 'lea' then 's.DimLeaId = org.DimLeaId'
				   else 's.DimSchoolId = org.DimSchoolId' end  
			+  '  
			where fact.DimCountDateId = @dimDateId  
			and fact.DimFactTypeId = @dimFactTypeId
			and ' + case when @reportLevel = 'lea' then 's.DimLeaId <> -1'
				    else 's.DimSchoolId <> -1' end  
	...
	--
	  ...
	  begin
		...
		set @sqlCountJoins = @sqlCountJoins + '
			inner join (
				select distinct fact.DimStudentId, IIF(fact.DimSchoolId > 0, fact.dimschoolid, fact.dimleaid) as OrgId, demo.DimDemographicId
				from rds.' + @factTable + ' fact
				inner join rds.DimSchools s on fact.dimschoolid = s.dimschoolid
				and fact.DimCountDateId = @dimDateId
				and fact.DimFactTypeId = @dimFactTypeId
				...
				inner join rds.DimDemographics demo on demo.DimDemographicId = fact.DimDemographicId
				where demo.HomelessStatusEdFactsCode = ''HOMELSENRL'' and statuses.McKinneyVentoSubgrantRecipientCode = ''Yes''
			) rules
				on fact.DimStudentId = rules.DimStudentId and fact.DimDemographicId =  rules.DimDemographicId and IIF(fact.DimSchoolId > 0, fact.dimschoolid, fact.dimleaid) = rules.OrgId'
		end
	...
	if(@factReportTable = 'FactPersonnelCountReports')
		begin
			...
			begin
				...
				insert into #categorySet
				(DimSchoolId, DimPersonnelId' + @sqlCategoryFields + ',PersonnelCount, ' + @factField + ')
				select distinct fact.DimSchoolId,fact.DimPersonnelId' + @sqlCategoryQualifiedDimensionFields + ',
				...
				and fact.DimFactTypeId = @dimFactTypeId ' + @queryFactFilter + '
				and fact.DimSchoolId <> -1
				group by fact.DimSchoolId,fact.DimPersonnelId' + @sqlCategoryQualifiedDimensionGroupFields + '
				' + @sqlHavingClause + '
				'
			end
			else
		...
		begin
			if(@reportCode = 'c006')
				...
				begin
					...
					create table #categorySet (	' 
					+ case when @reportLevel = 'lea' then 'DimLeaId int,' 
						   when @reportLevel = 'sch' then 'DimSchoolId int,'
						   else '' 
					end + '
					DimStudentId int' + @sqlCategoryFieldDefs + ', DisciplineDuration decimal(18, 2), 
						' + @factField + ' int
					)
					' + case when @reportLevel = 'lea' then '
					...
					(' + case when @reportLevel = 'lea' then 'DimLeaId,' 
							  when @reportLevel = 'sch' then 'DimSchoolId,'
							  else ''
					end + 'DimStudentId' 
					+ @sqlCategoryFields + ', DisciplineDuration, ' + @factField + ')
					select ' + case when @reportLevel = 'lea' then 'fact.DimLeaId,' 
									when @reportLevel = 'sch' then 'fact.DimSchoolId,'
									else ''
					...
					where fact.DimCountDateId = @dimDateId ' + @reportFilterCondition + '
					and fact.DimFactTypeId = @dimFactTypeId ' + @queryFactFilter + '
					and ' + case when @reportLevel = 'lea' then 'fact.DimLeaId <> -1'
								 when @reportLevel = 'sch' then 'IIF(fact.DimSchoolId > 0, fact.DimSchoolId, fact.DimLeaId) <> -1'
								 else '1 = 1' end  + '
					group by ' + case when @reportLevel = 'lea' then 'fact.DimLeaId,'
								 when @reportLevel = 'sch' then 'fact.DimSchoolId,'
								 else ''
			...
			else if(@reportCode in ('c005','c086','c144'))
				begin
							set @sql = @sql + '
			...
			begin
				...
				from rds.' + @factTable + ' fact ' + @sqlCountJoins 
				+ ' ' + @reportFilterJoin + '
				where IIF(fact.DimSchoolId > 0, fact.DimSchoolId, fact.DimLeaId) <> -1
				and fact.DimFactTypeId = @dimFactTypeId ' + @queryFactFilter + '
			...
			begin
				...
				begin
					...
					set @sql = @sql + '
						...
						where IIF(cs.DimSchoolId > 0, cs.DimSchoolId, cs.DimLeaId) <> -1
					'
				end
			else
		...
		if @reportLevel = 'sea'
			...
			begin
				...
				set @sql = @sql + '
					from #categorySet cs
					inner join rds.DimSeas sch on sch.DimSeaId <> -1
					group by 
						sch.StateANSICode,
						sch.StateCode,
						sch.StateName,
			...
			begin
				set @sql = @sql + '
					-- insert lea sql '
				if(@factReportTable = 'FactStudentCountReports' OR @reportCode = 'c088')
					begin
						set @sql = @sql + '
							insert into rds.' + @factReportTable + '
							(
	...
	begin
		...
		begin
			...
			inner join ( select OrganizationId' + @sqlCategoryFields + 
			' from @reportData
			group by OrganizationId,CategorySetCode' + @sqlCategoryFields +
			' having  CategorySetCode =  ''' + @categorySetCode + ''') b 
			on ' + @sqlPerformanceLevelJoins + ' and a.PERFORMANCELEVEL = b.PERFORMANCELEVEL and a.OrganizationId = b.OrganizationId
			'
