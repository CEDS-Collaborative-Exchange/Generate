...
BEGIN
	...
	declare @raceQuery as table (
		...
		--INDEX IX_Race NONCLUSTERED(DimStudentId, DimCountDateId, RaceRecordStartDate, RaceRecordEndDate)
	)
	declare @ageQuery as table (
		...
		DimCountDateId int,
		AgeCode varchar(50)
		--INDEX IX_Age NONCLUSTERED(DimStudentId, DimCountDateId)
	)
	...
	declare @demoQuery as table (
		...
		--INDEX IX_Demo NONCLUSTERED (DimStudentId, DimCountDateId, PersonStartDate, PersonEndDate)
	)
	declare @ideaQuery as table (
		...
		--INDEX IX_Idea NONCLUSTERED (DimStudentId, DimCountDateId, DimSchoolId, DimLeaId, RecordStartDateTime, RecordEndDateTime)
	)
	...
	declare @schoolQuery as table (
		...
		DimSeaId int,
		Organizationid int
		--unique clustered (DimStudentId, PersonId, DimSchoolId, DimCountDateId, DimLeaId, OrganizationId),
		--INDEX IX_Schools_Students NONCLUSTERED (DimStudentId, DimCountDateId),
		--INDEX IX_Schools NONCLUSTERED (DimSchoolId, DimLeaId)
	)
	...
	BEGIN
		...
		exec rds.Migrate_DimGradeLevels @studentDateQuery, @useCutOffDate
...
----Migrate_DimFirearms
	...
	inner join @schoolQuery sch 
		on s.DimStudentId = sch.DimStudentId 
		and s.PersonId = sch.PersonId
		and s.DimCountDateId = sch.DimCountDateId
	left outer join @disciplineQuery dis 
		on s.DimStudentId = dis.DimStudentId 
		and s.PersonId = dis.PersonId
		and s.DimCountDateId = dis.DimCountDateId 
		and sch.DimSchoolId = dis.DimSchoolId 
		and sch.DimLeaId = dis.DimLeaId
	left outer join @demoQuery demo 
		on s.DimStudentId = demo.DimStudentId 
		and s.PersonId = demo.PersonId
		and s.DimCountDateId = demo.DimCountDateId
	...
	left outer join @ageQuery a 
		on s.DimStudentId = a.DimStudentId 
		and s.PersonId = a.PersonId
		and s.DimCountDateId = a.DimCountDateId
	left outer join @raceQuery race 
		on s.DimStudentId = race.DimStudentId 
		and s.PersonId = race.PersonId
	...
	left outer join @gradelevelQuery grade 
		on s.DimStudentId = grade.DimStudentId 
		and s.DimCountDateId = grade.DimCountDateId 
		and sch.DimSchoolId = grade.DimSchoolId 
		and sch.DimLeaId = grade.DimLeaId
	left outer join @ideaQuery idea 
		on s.DimStudentId = idea.DimStudentId 
		and s.PersonId = idea.PersonId
		and s.DimCountDateId = idea.DimCountDateId 
		and idea.DimSchoolId = sch.DimSchoolId 
		and sch.DimLeaId = idea.DimLeaId
		and dis.DisciplinaryActionStartDate BETWEEN CAST(idea.RecordStartDateTime AS DATE) AND ISNULL(CAST(idea.RecordEndDateTime AS DATE), GETDATE())
	left outer join @programStatusQuery programStatus 
		on s.DimStudentId = programStatus.DimStudentId 
		and s.DimCountDateId = programStatus.DimCountDateId 
		and sch.DimSchoolId = programStatus.DimSchoolId 
		and sch.DimLeaId = programStatus.DimLeaId
	left outer join @firearmQuery firearms 
		on s.DimStudentId = firearms.DimStudentId 
		and s.DimCountDateId = firearms.DimCountDateId 
		and sch.DimSchoolId = firearms.DimSchoolId 
		and sch.DimLeaId = firearms.DimLeaId
	left outer join @cteStatusQuery cteStatus  
		on s.DimStudentId = cteStatus.DimStudentId 
		and s.DimCountDateId = cteStatus.DimCountDateId 
		and sch.DimSchoolId = cteStatus.DimSchoolId 
		and sch.DimLeaId = cteStatus.DimLeaId
	left outer join @disciplineFirearmsQuery disciplineFirearms 
		on s.DimStudentId = disciplineFirearms.DimStudentId 
		and s.DimCountDateId = disciplineFirearms.DimCountDateId 
		and sch.DimSchoolId = disciplineFirearms.DimSchoolId 
