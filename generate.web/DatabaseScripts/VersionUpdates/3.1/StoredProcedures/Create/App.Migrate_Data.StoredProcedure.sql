...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			from
			(select CAST(d.[YEAR] + 1 as smallint) as SY from rds.DimDateDataMigrationTypes ddt
			inner join rds.DimDataMigrationTypes r on ddt.DataMigrationTypeId = r.DimDataMigrationTypeId
			inner join rds.DimDates d on ddt.DimDateId = d.DimDateId
			where r.DataMigrationTypeCode = 'ods' and ddt.IsSelected = 1) years
			CROSS JOIN (SELECT StoredProcedureName
				FROM App.DataMigrationTasks
