CREATE PROCEDURE [Staging].[Migrate_Data_Validation]
...
Example Usage: 
    EXEC Staging.[Migrate_Data_Validation];
...
BEGIN
	...
	BEGIN	
		...
		-- Validate that all lookup values have a corresponding value in Staging.SourceSystemReferenceData
		...
		IF (@LookupTable IS NOT NULL AND @LookupTable <> '') BEGIN
			...
			SET @SQL = @SQL + 'SELECT ''' + @eStoredProc + ''', ''' + @eTable + ''', ''' + @ColumnName + ''', ''Source Value Not In CEDS Option Set'', ''No match for lookup value "'' + stg.' + @ColumnName + ' + ''" in Staging.SourceSystemReferenceData for Staging.' + @eTable + '.' + @ColumnName + ''', 4,' + @PrimaryKeyField + ', NULL '
			SET @SQL = @SQL + 'FROM Staging.' + @eTable + ' stg '
			SET @SQL = @SQL + 'LEFT JOIN Staging.SourceSystemReferenceData ref ON ref.TableName = ''' + @LookupTable + ''' AND InputCode = stg.' + @ColumnName + ' '
	...
	BEGIN	
		...
		(	
		    ...
		    , 'Staging.K12Organization'					-- TableName
