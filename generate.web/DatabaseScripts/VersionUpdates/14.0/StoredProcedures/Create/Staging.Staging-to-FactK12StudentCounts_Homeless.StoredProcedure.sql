...
BEGIN
	...
	--Create and load #Facts temp table
		...
		SELECT DISTINCT
			...
			, ISNULL(rds.Get_DimDate(ske.EnrollmentEntryDate), -1)	EnrollmentEntryDateId
			, ISNULL(rds.Get_DimDate(ske.EnrollmentExitDate), -1)	EnrollmentExitDateId
	...
	--dimpeople	(rds) - direct join to DimPeople_Current
		...
		LEFT JOIN RDS.DimLeas rdl
			...
			AND (rdl.RecordStartDateTime BETWEEN hmStatus.Homelessness_StatusStartDate AND ISNULL(hmStatus.Homelessness_StatusExitDate, @SYEndDate)
		...
		LEFT JOIN RDS.DimK12Schools rdksch
			...
			AND (rdksch.RecordStartDateTime BETWEEN hmStatus.Homelessness_StatusStartDate AND ISNULL(hmStatus.Homelessness_StatusExitDate, @SYEndDate)
	...
	--english learner
		LEFT JOIN #tempELStatus el 
			...
			AND hmStatus.Homelessness_StatusStartDate BETWEEN el.EnglishLearner_StatusStartDate AND ISNULL(el.EnglishLearner_StatusExitDate, @SYEndDate)
	--migratory status	
		LEFT JOIN #tempMigrantStatus migrant
			...
			AND hmStatus.Homelessness_StatusStartDate BETWEEN migrant.Migrant_StatusStartDate AND ISNULL(migrant.Migrant_StatusExitDate, @SYEndDate)
	--race	
		...
		BEGIN
			SELECT
				...
				, CASE
					...
					WHEN NOT EXISTS (
						SELECT 1
						FROM Staging.PersonStatus hmStatus
						WHERE ske.SchoolYear = hmStatus.SchoolYear
							AND ske.StudentIdentifierState = hmStatus.StudentIdentifierState
							AND ISNULL(ske.LeaIdentifierSeaAccountability, '') = ISNULL(hmStatus.LeaIdentifierSeaAccountability, '')
							AND ISNULL(ske.SchoolIdentifierSea, '') = ISNULL(hmStatus.SchoolIdentifierSea, '')
							AND hmStatus.Homelessness_StatusStartDate BETWEEN ske.EnrollmentEntryDate AND ISNULL(ske.EnrollmentExitDate, @SYEndDate)
							AND hmStatus.HomelessnessStatus = 1
					) THEN 'JOIN Staging.PersonStatus Homeless'
					...
					WHEN NOT EXISTS (SELECT 1 FROM RDS.DimAges rda WHERE RDS.Get_Age(ske.Birthdate, @SYStartDate) = rda.AgeValue) THEN 'JOIN RDS.DimAges'
					WHEN NOT EXISTS (
						SELECT 1
				...
				, CASE WHEN EXISTS (
					...
					WHERE ske.SchoolYear = hmStatus.SchoolYear
						...
						AND hmStatus.Homelessness_StatusStartDate BETWEEN ske.EnrollmentEntryDate AND ISNULL(ske.EnrollmentExitDate, @SYEndDate)
						AND hmStatus.HomelessnessStatus = 1
				) THEN 1 ELSE 0 END AS MatchedHomelessPersonStatus
				...
				, CASE WHEN EXISTS (SELECT 1 FROM RDS.DimAges rda WHERE RDS.Get_Age(ske.Birthdate, @SYStartDate) = rda.AgeValue) THEN 1 ELSE 0 END AS MatchedDimAges
				, CASE WHEN EXISTS (
					SELECT 1
					FROM RDS.vwDimK12Demographics rdkd
					WHERE ske.SchoolYear = rdkd.SchoolYear
