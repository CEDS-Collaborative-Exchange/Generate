...
*/
	...
	*/
		...
		begin
			...
			left join ( 
				select top(1) rds.DimDemographics.DimDemographicId, rds.DimDemographics.LepStatusCode from rds.DimDemographics where LepStatusCode = 'LEP'
				) as demo on demo.LepStatusCode=q.LepStatusCode
			left join ( 
				select top(1) rds.DimIdeaStatuses.DimIdeaStatusId, rds.DimIdeaStatuses.DisabilityCode from rds.DimIdeaStatuses where DisabilityCode = 'AUT'
				) as idea on idea.DisabilityCode=q.DisabilityCode
			left join ( 
				select top(1) rds.DimDemographics.DimDemographicId as DimEcoDisStatusId, rds.DimDemographics.EcoDisStatusCode from rds.DimDemographics where EcoDisStatusCode = 'EconomicDisadvantage'
