...
BEGIN
				...
				begin
					...
					begin
						set @sqlCategoryReturnField = ' 
							...
							when CAST(SUBSTRING( assmnt.PerformanceLevelEdFactsCode, 2,1) as int ) < CAST( tgglAssmnt.ProficientOrAboveLevel as int)  THEN  ''NOTPROFICIENT''														else ''MISSING''
							end'
					end
	...
	else if @reportCode in ('c033','c040')
		begin
		set @sqlCountJoins = @sqlCountJoins + '
			inner join (
				select distinct fact.DimStudentId
	...
	if(@factReportTable = 'FactPersonnelCountReports')
		...
		if @reportCode in ('c143')
		BEGIN
			
			declare @list as varchar(100)
			
			select @list = COALESCE(@list + ',', '') + item from app.Split(@sqlCategoryFields, ',') 
			where item not in ('DISCIPLINEACTION', 'REMOVALTYPE')
				
			set @sqlCategoryFields = ',' + @list
		...
		if @reportLevel = 'sea'
			...
			begin
				set @sql = @sql + '
					-- insert lea sql '
				if(@factReportTable = 'FactStudentCountReports' OR @reportCode in ('c088', 'c143'))
					begin
						set @sql = @sql + '
							insert into rds.' + @factReportTable + '
							(
