...
BEGIN
	...
	DROP TABLE IF EXISTS #C089Staging
	...
	select @ChildCountDate = convert(varchar, @CutoffMonth) + '/' + convert(varchar, @CutoffDay) + '/' + convert(varchar, @SchoolYear-1) -- < changed to "-1"
	 ...
	 SELECT  
		...
		left JOIN Staging.PersonStatus el 
			...
			AND ((ske.LEA_Identifier_State = el.OrganizationIdentifier
					AND el.OrganizationType = 'LEA')
				OR (ske.School_Identifier_State = el.OrganizationIdentifier
					AND el.OrganizationType = 'K12School'))
