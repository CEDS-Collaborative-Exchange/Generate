...
BEGIN
	...
	and upper(d.DimensionFieldName) not in ('REMOVALLENGTH','DISCIPLINEMETHOD','REMOVALREASON')
	...
	create table #reportCounts
		...
		DISCIPLINEACTION [nvarchar](100) NULL,
		DISCIPLINEMETHOD [nvarchar](100) NULL,
		REMOVALREASON [nvarchar](100) NULL,
		REMOVALTYPE [nvarchar](100) NULL,
	...
	SELECT @zeroCountSQL = [RDS].Get_CountSQL (@reportCode, @reportLevel, @reportYear, @categorySetCode, 'zero-discipline', 1, 0,'','')
	...
	SELECT 			CAST(ROW_NUMBER() OVER(ORDER BY Category ASC) AS INT) as FactCustomCountId,
					...
					CAST(SUM(CASE WHEN REMOVALTYPE = 'REMHO' THEN DisciplineCount ELSE 0 END) as decimal(18,2)) as col_1,
					CAST(SUM(CASE WHEN REMOVALTYPE = 'REMDW' THEN DisciplineCount ELSE 0 END) as decimal(18,2)) as col_2,
					CAST(SUM(CASE WHEN REMOVALREASON = 'W' THEN DisciplineCount ELSE 0 END) as decimal(18,2)) as col_3,
					CAST(SUM(CASE WHEN REMOVALREASON = 'D' THEN DisciplineCount ELSE 0 END) as decimal(18,2)) as col_4,
					CAST(SUM(CASE WHEN REMOVALREASON = 'SBI' THEN DisciplineCount ELSE 0 END) as decimal(18,2)) as col_5,
					CAST(SUM(CASE WHEN REMOVALLENGTHIDEA = 'LESS10' AND REMOVALTYPE = 'REMHO' THEN DisciplineCount ELSE 0 END) as decimal(18,2)) as col_6,
					CAST(SUM(CASE WHEN REMOVALLENGTHIDEA = 'GREATER10' AND REMOVALTYPE = 'REMHO' THEN DisciplineCount ELSE 0 END) as decimal(18,2)) as col_7,
					CAST(SUM(CASE WHEN REMOVALLENGTHIDEA = 'LESS10' AND REMOVALTYPE = 'REMDW' THEN DisciplineCount ELSE 0 END) as decimal(18,2)) as col_8,
					CAST(SUM(CASE WHEN REMOVALLENGTHIDEA = 'GREATER10' AND REMOVALTYPE = 'REMDW' THEN DisciplineCount ELSE 0 END) as decimal(18,2)) as col_9,
