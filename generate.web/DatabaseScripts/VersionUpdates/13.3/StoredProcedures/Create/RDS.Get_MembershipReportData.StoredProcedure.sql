CREATE PROCEDURE [RDS].[Get_MembershipReportData]
...
-- Determine Fact/Report Tables
	...
	set @endRecord = @startRecord + @numberOfRecords
...
*/
	...
	set @sql = @sql + ' 
		select * from 
		( select rowid = ROW_NUMBER() OVER (ORDER BY ' + @factReportId + '),' +
		 @factReportId + ',' + '''' + @reportCode + ''' as ReportCode'
	...
	set @sql = @sql + '	from @reportData ) r
		where rowid >= ' + CAST(@startRecord as varchar(10)) + ' AND rowid < ' + CAST(@endRecord as varchar(10)) + '  
			'
