...
### What submitted E&#x44;_&#x46;acts_ Files are included in the <mark style="color:blue;">Membership Fact Type</mark>?
...
{% code overflow="wrap" %}
```sql
--What EDFacts Files are included in the Membership Fact Type?
...
{% hint style="success" %}
The following script will return the needed staging table, and columns for **Membership**:
{% endhint %}
...
FROM app.vwStagingRelationships
WHERE FactTypeCode = 'membership' and ReportCode = '033'
ORDER BY FactTypeCode, ReportCode, StagingTableName, StagingcolumnName
```
{% endcode %}

#### Source System Reference Tables Membership Filters
...
Membership Reports ('033', '052', '226') have filters
...
{% code overflow="wrap" %}
```sql
exec [Utilities].[Check_SourceSystemReferenceData_Mapping] 'membership', '2024', 0 -- This will show all mappings for the Membership Fact Type for School Year 2023-24
```
{% endcode %}
...
For Membership , this Stored Procedure is called `[Source].[Source-to-Staging_Membership].`
...
```sql
exec [Source].[Source-to-Staging_Membership] 2024
```
{% endtab %}
...
{% hint style="success" %}
The following is an example code snippet of how to call these Stored Procedures by the Membership Fact Type.
{% endhint %}

```sql
exec [Staging].[StagingValidation_Execute] 2024,'membership'
exec [Staging].[StagingValidation_GetResults] 2024,'membership'
```
...
The following is an example code snippet of how to select the Membership Staging Table Debug view:

```sql
select * from [debug].[vwMembership_StagingTables]
```
...
To migrate data from Staging to the CEDS Data Warehouse you will need to call the \[App].\[Wrapper\_Migrate\_Membership\_to\_RDS] Stored Procedure. This wrapper will call several Stored Procedures to migrate data to the dimension and fact tables in the CEDS Data Warehouse as well as log this activity in the App.DataMigrationHistories table. This process will also create debug tables that contain the information that is utilized in the counts and can be used in the validation process.
...
exec [app].[Wrapper_Migrate_Membership_to_RDS]
```
{% endcode %}
{% endtab %}
...
```sql
select * from [debug].[vwMembership_FactTable]
```
...
{% code overflow="wrap" %}
```sql
exec [Staging].[Staging-to-FactK12StudentCounts_Membership] 2026, '123456789', 1
```
{% endcode %}
...
-- B. Lock the reports to be run
    ...
    SET IsLocked = 1
    WHERE ReportCode IN ('033', '052', '226')

-- C. Empty the reports table for the specific reports    
    EXEC [rds].[Empty_Reports] @FactTypeCode = 'membership'

-- D. Perform the reports migration
    EXEC [rds].[create_reports] 'membership',0
...
@ReportCode = 'C033', -- EdFacts File Number – 033 , 052, 226
@ReportLevel = 'LEA', -- 'SEA', 'LEA', 'SCH'
@LegacyTableName = 'Generate.XX.033_LEA_2023_Legacy', -- Legacy table
@GenerateTableName = 'Generate.XX.033_LEA_2023_Generate', -- Generate table
@ShowSQL = 0,
@ComparisonResultsTableName = 'generate.XX.033_LEA_2023_COMPARISON' -- Comparison table name
```
{% endcode %}
