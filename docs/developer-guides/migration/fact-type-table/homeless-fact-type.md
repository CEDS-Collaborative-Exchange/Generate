...
### What submitted E&#x44;_&#x46;acts_ Files are included in the <mark style="color:blue;">Homeless Fact Type</mark>?

The following files have been created in Generate and submitted to E&#x44;_&#x46;acts_:

* [x] FS118: Homeless Students Enrolled
* [x] FS194: Young Homeless Children Served (McKinney-Vento)

### What E&#x44;_&#x46;acts_ Files in the <mark style="color:blue;">Homeless Fact Type</mark> are available for pilot opportunity?

The following files are in pilot status or are available for piloting in Generate:

* [ ] FS118: Homeless Students Enrolled
* [ ] FS194: Young Homeless Children Served (McKinney-Vento)

{% code overflow="wrap" %}
```sql
--What EDFacts Files are included in the Homeless Fact Type?
...
WHERE           rdft.FactTypeCode = 'homeless'
...
{% hint style="success" %}
The following script will return the needed staging table, and columns for **Homeless**:
{% endhint %}
...
FROM app.vwStagingRelationships
WHERE FactTypeCode = 'homeless' and ReportCode = '118'
ORDER BY FactTypeCode, ReportCode, StagingTableName, StagingcolumnName
```
{% endcode %}

**Source System Reference Tables Homeless Filters**
...
Homeless Reports ('118','194') have filters
...
{% code overflow="wrap" %}
```sql
exec [Utilities].[Check_SourceSystemReferenceData_Mapping] 'homeless', '2024', 0 -- This will show all mappings for the Homeless Fact Type for School Year 2023-24
```
{% endcode %}
...
```
/****** Object:  StoredProcedure [Source].[Source-to-Staging_Homeless]    Script Date: 8/18/2025 10:12:03 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [Source].[Source-to-Staging_Homeless] 
	@schoolYear smallint
AS
--BEGIN
...
```sql
exec [Source].[Source-to-Staging_Homeless] 2024
```
{% endtab %}
...
{% hint style="success" %}
The following is an example code snippet of how to call these Stored Procedures by the Homeless Fact Type.
{% endhint %}

```sql
exec [Staging].[StagingValidation_Execute] 2024,'homeless'
exec [Staging].[StagingValidation_GetResults] 2024,'homeless'
```
...
The following is an example code snippet of how to select the Homeless Staging Table Debug view:

```sql
select * from [debug].[vwHomeless_StagingTables]
```
...
To migrate data from Staging to the CEDS Data Warehouse you will need to call the `[App].[Wrapper_Migrate_Homeless_to_RDS]` Stored Procedure. This wrapper will call several Stored Procedures to migrate data to the dimension and fact tables in the CEDS Data Warehouse as well as log this activity in the `App.DataMigrationHistories` table. This process will also create debug tables that contain the information that is utilized in the counts and can be used in the validation process.
...
exec [app].[Wrapper_Migrate_Homeless_to_RDS]
```
{% endcode %}
{% endtab %}
...
```sql
select * from [debug].[vwHomeless_FactTable]
```
...
{% code overflow="wrap" %}
```sql
exec [Staging].[Staging-to-FactK12StudentCounts_Homeless] 2026, '123456789', 1
```
{% endcode %}
...
-- B. Lock the reports to be run
    ...
    SET IsLocked = 1
    WHERE ReportCode IN ('118','194')

-- C. Empty the reports table for the specific reports    
    EXEC [rds].[Empty_Reports] @FactTypeCode = 'homeless'

-- D. Perform the reports migration
    EXEC [rds].[create_reports] 'homeless',0
...
SELECT * FROM [debug].[118_lea_ST1_2023_BASISEXIT]
```
...
@ReportCode = '118', -- EdFacts File Number – '118','194'
@ReportLevel = 'LEA', -- 'SEA', 'LEA', 'SCH'
@LegacyTableName = 'Generate.XX.118_LEA_2023_Legacy', -- Legacy table
@GenerateTableName = 'Generate.XX.118_LEA_2023_Generate', -- Generate table
@ShowSQL = 0,
@ComparisonResultsTableName = 'generate.XX.118_LEA_2023_COMPARISON' -- Comparison table name
```
{% endcode %}
