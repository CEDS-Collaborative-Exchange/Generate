...
### What submitted E&#x44;_&#x46;acts_ Files are included in the <mark style="color:blue;">Immigrant Fact Type</mark>?

The following files have been created in Generate and submitted to E&#x44;_&#x46;acts_:

* [x] FS045: Immigrant
* [x] FS116: Title III Students Served
* [x] FS210: Title III English Leaner Five Years
* [x] FS211: Title III English Leaner Exited

### What E&#x44;_&#x46;acts_ Files in the <mark style="color:blue;">Immigrant Fact Type</mark> are available for pilot opportunity?

The following files are in pilot status or are available for piloting in Generate:

* [ ] FS045: Immigrant
* [ ] FS116: Title III Students Served
* [ ] FS210: Title III English Leaner Five Years
* [ ] FS211: Title III English Leaner Exited

{% code overflow="wrap" %}
```sql
--What EDFacts Files are included in the Immigrant Fact Type?
...
WHERE           rdft.FactTypeCode = 'immigrant'
...
{% hint style="success" %}
The following script will return the needed staging table, and columns for **Immigrant**:
{% endhint %}
...
FROM app.vwStagingRelationships
WHERE FactTypeCode = 'immigrant' and ReportCode = '045'
ORDER BY FactTypeCode, ReportCode, StagingTableName, StagingcolumnName
```
{% endcode %}

**Source System Reference Tables Immigrant Filters**
...
Immigrant Reports (045,116,210,211) have filters
...
{% code overflow="wrap" %}
```sql
exec [Utilities].[Check_SourceSystemReferenceData_Mapping] 'immigrant', '2024', 0 -- This will show all mappings for the Immigrant Fact Type for School Year 2023-24
```
{% endcode %}
...
<figure><img src="../../../.gitbook/assets/image (228).png" alt="SQL Server Management Studio showing the Source-to-Staging Immigrant stored procedure placeholder."><figcaption><p>Screenshot of the Generate database structure in SQL Server Management Studio, showing a stored procedure placeholder for the "Source-to-Staging_(Fact Type Name)" Fact Type.</p></figcaption></figure>
...
```
/****** Object:  StoredProcedure [Source].[Source-to-Staging_Immigrant]    Script Date: 8/18/2025 10:12:03 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [Source].[Source-to-Staging_Immigrant] 
	@schoolYear smallint
AS
--BEGIN
...
```sql
exec [Source].[Source-to-Staging_Immigrant] 2024
```
{% endtab %}
...
{% hint style="success" %}
The following is an example code snippet of how to call these Stored Procedures by the Immigrant Fact Type.
{% endhint %}

```sql
exec [Staging].[StagingValidation_Execute] 2024,'immigrant'
exec [Staging].[StagingValidation_GetResults] 2024,'immigrant'
```
...
The following is an example code snippet of how to select the Immigrant Staging Table Debug view:

```sql
select * from [debug].[vwImmigrant_StagingTables]
```
...
exec [app].[Wrapper_Migrate_Immigrant_to_RDS]
```
{% endcode %}
{% endtab %}
...
```sql
select * from [debug].[vwImmigrant_FactTable]
```
...
-- B. Lock the reports to be run
    ...
    SET IsLocked = 1
    WHERE ReportCode IN (045,116,210,211)

-- C. Empty the reports table for the specific reports    
    EXEC [rds].[Empty_Reports] @FactTypeCode = 'immigrant'

-- D. Perform the reports migration
    EXEC [rds].[create_reports] 'immigrant',0
...
@ReportCode = '045', -- EdFacts File Number – 045,116,210,211
