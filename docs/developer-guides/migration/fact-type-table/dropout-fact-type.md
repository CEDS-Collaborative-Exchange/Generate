...
### What submitted E&#x44;_&#x46;acts_ Files are included in the <mark style="color:blue;">Dropout Fact Type</mark>?

The following files have been created in Generate and submitted to E&#x44;_&#x46;acts_:

* [x] FS032: Dropouts

### What E&#x44;_&#x46;acts_ Files in the <mark style="color:blue;">Dropout Fact Type</mark> are available for pilot opportunity?
...
{% code overflow="wrap" %}
```sql
--What EDFacts Files are included in the Dropout Fact Type?
...
WHERE           rdft.FactTypeCode = 'dropout'
...
{% hint style="success" %}
The following script will return the needed staging table, and columns for **Dropout**:
{% endhint %}
...
FROM app.vwStagingRelationships
WHERE FactTypeCode = 'dropout' and ReportCode = '032'
ORDER BY FactTypeCode, ReportCode, StagingTableName, StagingcolumnName
```
{% endcode %}

**Source System Reference Tables Dropout Filters**
...
Dropout Reports (032) have filters
...
{% code overflow="wrap" %}
```sql
exec [Utilities].[Check_SourceSystemReferenceData_Mapping] 'dropout', '2024', 0 -- This will show all mappings for the Dropout Fact Type for School Year 2023-24
```
{% endcode %}
...
```
/****** Object:  StoredProcedure [Source].[Source-to-Staging_Dropout]    Script Date: 8/18/2025 10:12:03 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [Source].[Source-to-Staging_Dropout] 
	@schoolYear smallint
AS
--BEGIN
...
```sql
exec [Source].[Source-to-Staging_Dropout] 2024
```
{% endtab %}
...
{% hint style="success" %}
The following is an example code snippet of how to call these Stored Procedures by the Dropout Fact Type.
{% endhint %}

```sql
exec [Staging].[StagingValidation_Execute] 2024,'dropout'
exec [Staging].[StagingValidation_GetResults] 2024,'dropout'
```
...
The following is an example code snippet of how to select the Dropout Staging Table Debug view:

```sql
select * from [debug].[vwDropout_StagingTables]
```
...
To migrate data from Staging to the CEDS Data Warehouse you will need to call the `[App].[Wrapper_Migrate_Dropout_to_RDS]` Stored Procedure. This wrapper will call several Stored Procedures to migrate data to the dimension and fact tables in the CEDS Data Warehouse as well as log this activity in the `App.DataMigrationHistories` table. This process will also create debug tables that contain the information that is utilized in the counts and can be used in the validation process.
...
exec [app].[Wrapper_Migrate_Dropout_to_RDS]
```
{% endcode %}
{% endtab %}
...
```sql
select * from [debug].[vwDropout_FactTable]
```
...
-- B. Lock the reports to be run
    ...
    SET IsLocked = 1
    WHERE ReportCode IN (032)

-- C. Empty the reports table for the specific reports    
    EXEC [rds].[Empty_Reports] @FactTypeCode = 'dropout'

-- D. Perform the reports migration
    EXEC [rds].[create_reports] 'dropout',0
...
SELECT * FROM [debug].[032_lea_ST1_2023_BASISEXIT]
```
...
@ReportCode = '032', -- EdFacts File Number – 032
@ReportLevel = 'LEA', -- 'SEA', 'LEA', 'SCH'
@LegacyTableName = 'Generate.XX.032_LEA_2023_Legacy', -- Legacy table
@GenerateTableName = 'Generate.XX.032_LEA_2023_Generate', -- Generate table
@ShowSQL = 0,
@ComparisonResultsTableName = 'generate.XX.032_LEA_2023_COMPARISON' -- Comparison table name
```
{% endcode %}
