...
description: >-
  ...
  for the Chronic Absenteeism Fact Type
---

# Chronic Absenteeism Fact Type
...
### What submitted E&#x44;_&#x46;acts_ Files are included in the <mark style="color:blue;">Chronic Absenteeism Fact Type</mark>?

The following files have been created in Generate and submitted to E&#x44;_&#x46;acts_:

* [x] FS195: Chronic Absenteeism

### What E&#x44;_&#x46;acts_ Files in the <mark style="color:blue;">Chronic Absenteeism Fact Type</mark> are available for pilot opportunity?

The following files are in pilot status or are available for piloting in Generate:

* [ ] FS195: Chronic Absenteeism

{% code overflow="wrap" %}
```sql
--What EDFacts Files are included in the Chronic Absenteeism Fact Type?
...
WHERE           rdft.FactTypeCode = 'chronicabsenteeism'
...
{% hint style="success" %}
The following script will return the needed staging table, and columns for **Chronic Absenteeism**:
{% endhint %}
...
WHERE FactTypeCode = 'chronicabsenteeism'
...
FROM app.vwStagingRelationships
WHERE FactTypeCode = 'chronicabsenteeism' and ReportCode = '195'
ORDER BY FactTypeCode, ReportCode, StagingTableName, StagingcolumnName
```
{% endcode %}

**Source System Reference Tables Chronic Absenteeism Filters**
...
Chronic Absenteeism Reports (195) have filters
...
{% code overflow="wrap" %}
```sql
exec [Utilities].[Check_SourceSystemReferenceData_Mapping] 'chronicabsenteeism', '2024', 0 -- This will show all mappings for the Chronic Absenteeism Fact Type for School Year 2023-24
```
{% endcode %}
...
<figure><img src="../../../.gitbook/assets/image (229).png" alt="SQL Server Management Studio showing the Source-to-Staging Chronic Absenteeism stored procedure placeholder."><figcaption><p>Screenshot of the Generate database structure in SQL Server Management Studio, showing a stored procedure placeholder for the "Source-to-Staging_(Fact Type Name)" Fact Type.</p></figcaption></figure>
...
```
/****** Object:  StoredProcedure [Source].[Source-to-Staging_ChronicAbsenteeism]    Script Date: 8/18/2025 10:12:03 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [Source].[Source-to-Staging_ChronicAbsenteeism] 
	@schoolYear smallint
AS
--BEGIN
...
```sql
exec [Source].[Source-to-Staging_ChronicAbsenteeism] 2024
```
{% endtab %}
...
{% hint style="success" %}
The following is an example code snippet of how to call these Stored Procedures by the Chronic Absenteeism Fact Type.
{% endhint %}

```sql
exec [Staging].[StagingValidation_Execute] 2024,'chronicabsenteeism'
exec [Staging].[StagingValidation_GetResults] 2024,'chronicabsenteeism'
```
...
The following is an example code snippet of how to select the Chronic Absenteeism Staging Table Debug view:

```sql
select * from [debug].[vwChronicAbsenteeism_StagingTables]
```
...
exec [app].[Wrapper_Migrate_ChronicAbsenteeism_to_RDS]
```
{% endcode %}
{% endtab %}
...
```sql
select * from [debug].[vwChronicAbsenteeism_FactTable]
```
...
{% code overflow="wrap" %}
```sql
exec [Staging].[Staging-to-FactK12StudentCounts_ChronicAbsenteeism] 2026, '123456789', 1
```
{% endcode %}
...
-- B. Lock the reports to be run
    ...
    SET IsLocked = 1
    WHERE ReportCode IN (195)

-- C. Empty the reports table for the specific reports    
    EXEC [rds].[Empty_Reports] @FactTypeCode = 'chronicabsenteeism'

-- D. Perform the reports migration
    EXEC [rds].[create_reports] 'chronicabsenteeism',0
...
SELECT * FROM [debug].[195_lea_ST1_2023_BASISEXIT]
```
...
@ReportCode = '195', -- EdFacts File Number – 195
@ReportLevel = 'LEA', -- 'SEA', 'LEA', 'SCH'
@LegacyTableName = 'Generate.XX.195_LEA_2023_Legacy', -- Legacy table
@GenerateTableName = 'Generate.XX.195_LEA_2023_Generate', -- Generate table
@ShowSQL = 0,
@ComparisonResultsTableName = 'generate.XX.195_LEA_2023_COMPARISON' -- Comparison table name
```
{% endcode %}
