...
### What submitted E&#x44;_&#x46;acts_ Files are included in the <mark style="color:blue;">Title I Fact Type</mark>?

The following files have been created in Generate and submitted to E&#x44;_&#x46;acts_:

* [x] FS037: Title I Part A SWP/TAS Participation
* [x] FS134: Title I Part A Participation
* [x] FS222: Foster Care Enrolled

### What E&#x44;_&#x46;acts_ Files in the <mark style="color:blue;">Title I Fact Type</mark> are available for pilot opportunity?
...
* [ ] FS134: Title I Part A Participation
* [ ] FS222: Foster Care Enrolled

{% code overflow="wrap" %}
```sql
--What EDFacts Files are included in the Title I Fact Type?
...
WHERE           rdft.FactTypeCode = 'titleI'
...
{% hint style="success" %}
The following script will return the needed staging table, and columns for **Title I**:
{% endhint %}
...
FROM app.vwStagingRelationships
WHERE FactTypeCode = 'titleI' and ReportCode = '037'
ORDER BY FactTypeCode, ReportCode, StagingTableName, StagingcolumnName
```
{% endcode %}

**Source System Reference Tables Title I Filters**
...
Title I Reports ('037','134','222') have filters
...
{% code overflow="wrap" %}
```sql
exec [Utilities].[Check_SourceSystemReferenceData_Mapping] 'titleI', '2024', 0 -- This will show all mappings for the Title I Fact Type for School Year 2023-24
```
{% endcode %}
...
```
/****** Object:  StoredProcedure [Source].[Source-to-Staging_TitleI]    Script Date: 8/18/2025 10:12:03 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [Source].[Source-to-Staging_TitleI] 
	@schoolYear smallint
AS
--BEGIN
...
```sql
exec [Source].[Source-to-Staging_TitleI] 2024
```
{% endtab %}
...
{% hint style="success" %}
The following is an example code snippet of how to call these Stored Procedures by the Title I Fact Type.
{% endhint %}

```sql
exec [Staging].[StagingValidation_Execute] 2024,'titleI'
exec [Staging].[StagingValidation_GetResults] 2024,'titleI'
```
...
The following is an example code snippet of how to select the Title I Staging Table Debug view:

```sql
select * from [debug].[vwTitleI_StagingTables]
```
...
To migrate data from Staging to the CEDS Data Warehouse you will need to call the `[App].[Wrapper_Migrate_TitleI_to_RDS]` Stored Procedure. This wrapper will call several Stored Procedures to migrate data to the dimension and fact tables in the CEDS Data Warehouse as well as log this activity in the `App.DataMigrationHistories` table. This process will also create debug tables that contain the information that is utilized in the counts and can be used in the validation process.
...
exec [app].[Wrapper_Migrate_TitleI_to_RDS]
```
{% endcode %}
{% endtab %}
...
```sql
select * from [debug].[vwTitleI_FactTable]
```
...
{% code overflow="wrap" %}
```sql
exec [Staging].[Staging-to-FactK12StudentCounts_TitleI] 2026, '123456789', 1
```
{% endcode %}
...
-- B. Lock the reports to be run
    ...
    SET IsLocked = 1
    WHERE ReportCode IN ('037','134','222')

-- C. Empty the reports table for the specific reports    
    EXEC [rds].[Empty_Reports] @FactTypeCode = 'titleI'

-- D. Perform the reports migration
    EXEC [rds].[create_reports] 'titleI',0
...
SELECT * FROM [debug].[037_lea_ST1_2023_BASISEXIT]
```
...
@ReportCode = '037', -- EdFacts File Number – '037','134','222'
@ReportLevel = 'LEA', -- 'SEA', 'LEA', 'SCH'
@LegacyTableName = 'Generate.XX.037_LEA_2023_Legacy', -- Legacy table
@GenerateTableName = 'Generate.XX.037_LEA_2023_Generate', -- Generate table
@ShowSQL = 0,
@ComparisonResultsTableName = 'generate.XX.037_LEA_2023_COMPARISON' -- Comparison table name
```
{% endcode %}
