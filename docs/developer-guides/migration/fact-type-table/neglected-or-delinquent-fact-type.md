...
description: >-
  ...
  for the Neglected or Delinquent Fact Type
---

# Neglected or Delinquent Fact Type
...
### What submitted E&#x44;_&#x46;acts_ Files are included in the <mark style="color:blue;">Neglected or Delinquent Fact Type</mark>?

The following files have been created in Generate and submitted to E&#x44;_&#x46;acts_:

* [x] FS119: Neglected or Delinquent Participation (SEA)
* [x] FS127: N or D Participation - LEA
* [x] FS218: N or D in Program Outcomes - State Agency
* [x] FS219: N or D in Program Outcomes - LEA
* [x] FS220: N or D Exit Outcomes - State Agency
* [x] FS221: N or D Exit Outcomes - LEA

### What E&#x44;_&#x46;acts_ Files in the <mark style="color:blue;">Neglected or Delinquent Fact Type</mark> are available for pilot opportunity?

The following files are in pilot status or are available for piloting in Generate:

* [ ] FS119: Neglected or Delinquent Participation (SEA)
* [ ] FS127: N or D Participation - LEA
* [ ] FS218: N or D in Program Outcomes - State Agency
* [ ] FS219: N or D in Program Outcomes - LEA
* [ ] FS220: N or D Exit Outcomes - State Agency
* [ ] FS221: N or D Exit Outcomes - LEA

{% code overflow="wrap" %}
```sql
--What EDFacts Files are included in the Neglected or Delinquent Fact Type?
...
WHERE           rdft.FactTypeCode = 'neglectedordelinquent'
...
{% hint style="success" %}
The following script will return the needed staging table, and columns for **Neglected or Delinquent**:
{% endhint %}
...
FROM app.vwStagingRelationships
WHERE FactTypeCode = 'neglectedordelinquent' and ReportCode = '119'
ORDER BY FactTypeCode, ReportCode, StagingTableName, StagingcolumnName
```
{% endcode %}

**Source System Reference Tables Neglected or Delinquent Filters**
...
Neglected or Delinquent Reports ('119','127','218','219','220','221') have filters
...
{% code overflow="wrap" %}
```sql
exec [Utilities].[Check_SourceSystemReferenceData_Mapping] 'neglectedordelinquent', '2024', 0 -- This will show all mappings for the Neglected or Delinquent Fact Type for School Year 2023-24
```
{% endcode %}
...
CREATE PROCEDURE [Source].[Source-to-Staging_NeglectedOrDelinquent] 
	@schoolYear smallint
AS
--BEGIN
...
```sql
exec [Source].[Source-to-Staging_NeglectedOrDelinquent] 2024
```
{% endtab %}
...
{% hint style="success" %}
The following is an example code snippet of how to call these Stored Procedures by the Neglected or Delinquent Fact Type.
{% endhint %}

```sql
exec [Staging].[StagingValidation_Execute] 2024,'neglectedordelinquent'
exec [Staging].[StagingValidation_GetResults] 2024,'neglectedordelinquent'
```
...
The following is an example code snippet of how to select the Neglected or Delinquent Staging Table Debug view:

```sql
select * from [debug].[vwNeglectedOrDelinquent_StagingTables]
```
...
To migrate data from Staging to the CEDS Data Warehouse you will need to call the `[App].[Wrapper_Migrate_NeglectedOrDelinquent_to_RDS]` Stored Procedure. This wrapper will call several Stored Procedures to migrate data to the dimension and fact tables in the CEDS Data Warehouse as well as log this activity in the `App.DataMigrationHistories` table. This process will also create debug tables that contain the information that is utilized in the counts and can be used in the validation process.
...
exec [app].[Wrapper_Migrate_NeglectedOrDelinquent_to_RDS]
```
{% endcode %}
{% endtab %}
...
-- B. Lock the reports to be run
    ...
    SET IsLocked = 1
    WHERE ReportCode IN ('119','127','218','219','220','221')

-- C. Empty the reports table for the specific reports    
    EXEC [rds].[Empty_Reports] @FactTypeCode = 'neglectedordelinquent'

-- D. Perform the reports migration
    EXEC [rds].[create_reports] 'neglectedordelinquent',0
...
SELECT * FROM [debug].[119_lea_ST1_2023_BASISEXIT]
```
...
@ReportCode = '119', -- EdFacts File Number – '119','127','218','219','220','221'
@ReportLevel = 'LEA', -- 'SEA', 'LEA', 'SCH'
@LegacyTableName = 'Generate.XX.119_LEA_2023_Legacy', -- Legacy table
@GenerateTableName = 'Generate.XX.119_LEA_2023_Generate', -- Generate table
@ShowSQL = 0,
@ComparisonResultsTableName = 'generate.XX.119_LEA_2023_COMPARISON' -- Comparison table name
```
{% endcode %}
