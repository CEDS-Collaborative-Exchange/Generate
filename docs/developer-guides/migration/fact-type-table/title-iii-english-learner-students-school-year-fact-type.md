...
description: >-
  ...
  for the Title III English Learner Students - School Year Fact Type
---

# Title III English Learner Students - School Year Fact Type
...
### What submitted E&#x44;_&#x46;acts_ Files are included in the <mark style="color:blue;">Title III English Learner Students - School Year Fact Type</mark>?

The following files have been created in Generate and submitted to E&#x44;_&#x46;acts_:

* [x] FS045: Immigrant
* [x] FS116: Title III Students Served

### What E&#x44;_&#x46;acts_ Files in the <mark style="color:blue;">Title III English Learner Students - School Year Fact Type</mark> are available for pilot opportunity?
...
{% code overflow="wrap" %}
```sql
--What EDFacts Files are included in the Title III English Learner Students - School Year Fact Type?
...
WHERE           rdft.FactTypeCode = 'titleIIIELSY'
...
{% hint style="success" %}
The following script will return the needed staging table, and columns for **Title III English Learner Students - School Year**:
{% endhint %}
...
FROM app.vwStagingRelationships
WHERE FactTypeCode = 'titleIIIELSY' and ReportCode = '045'
ORDER BY FactTypeCode, ReportCode, StagingTableName, StagingcolumnName
```
{% endcode %}

**Source System Reference Tables Title III English Learner Students - School Year Filters**
...
Title III English Learner Students - School Year Reports ('045','116') have filters
...
{% code overflow="wrap" %}
```sql
exec [Utilities].[Check_SourceSystemReferenceData_Mapping] 'titleIIIELSY', '2024', 0 -- This will show all mappings for the Title III English Learner Students - School Year Fact Type for School Year 2023-24
```
{% endcode %}
...
```
/****** Object:  StoredProcedure [Source].[Source-to-Staging_TitleIIIELSY]    Script Date: 8/18/2025 10:12:03 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [Source].[Source-to-Staging_TitleIIIELSY] 
	@schoolYear smallint
AS
--BEGIN
...
```sql
exec [Source].[Source-to-Staging_TitleIIIELSY] 2024
```
{% endtab %}
...
{% hint style="success" %}
The following is an example code snippet of how to call these Stored Procedures by the Title III English Learner Students - School Year Fact Type.
{% endhint %}

```sql
exec [Staging].[StagingValidation_Execute] 2024,'titleIIIELSY'
exec [Staging].[StagingValidation_GetResults] 2024,'titleIIIELSY'
```
...
The following is an example code snippet of how to select the Title III English Learner Students - School Year Staging Table Debug view:

```sql
select * from [debug].[vwTitleIIIELSY_StagingTables]
```
...
To migrate data from Staging to the CEDS Data Warehouse you will need to call the `[App].[Wrapper_Migrate_TitleIIIELSY_to_RDS]` Stored Procedure. This wrapper will call several Stored Procedures to migrate data to the dimension and fact tables in the CEDS Data Warehouse as well as log this activity in the `App.DataMigrationHistories` table. This process will also create debug tables that contain the information that is utilized in the counts and can be used in the validation process.
...
exec [app].[Wrapper_Migrate_TitleIIIELSY_to_RDS]
```
{% endcode %}
{% endtab %}
...
```sql
select * from [debug].[vwTitleIIIELSY_FactTable]
```
...
{% code overflow="wrap" %}
```sql
exec [Staging].[Staging-to-FactK12StudentCounts_TitleIIIELSY] 2026, '123456789', 1
```
{% endcode %}
...
-- B. Lock the reports to be run
    ...
    SET IsLocked = 1
    WHERE ReportCode IN ('045','116')

-- C. Empty the reports table for the specific reports    
    EXEC [rds].[Empty_Reports] @FactTypeCode = 'titleIIIELSY'

-- D. Perform the reports migration
    EXEC [rds].[create_reports] 'titleIIIELSY',0
...
SELECT * FROM [debug].[045_lea_ST1_2023_BASISEXIT]
```
...
@ReportCode = '045', -- EdFacts File Number – '045','116'
@ReportLevel = 'LEA', -- 'SEA', 'LEA', 'SCH'
@LegacyTableName = 'Generate.XX.045_LEA_2023_Legacy', -- Legacy table
@GenerateTableName = 'Generate.XX.045_LEA_2023_Generate', -- Generate table
@ShowSQL = 0,
@ComparisonResultsTableName = 'generate.XX.045_LEA_2023_COMPARISON' -- Comparison table name
```
{% endcode %}
