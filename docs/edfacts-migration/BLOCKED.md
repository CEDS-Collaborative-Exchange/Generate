...
### FS052 (Membership by Grade/Race/Sex) — report ACTUAL fully populates; oracle pending
After the populator fix, adding the per-dimension EdFacts-code columns to `RDS.vwMembership_FactTable_052` (`GradeLevelEdFactsCode`, `RaceEdFactsCode`, `SexEdFactsCode` — the last required adding `Demo.SexEdFactsCode` to `debug.vwMembership_FactTable`) makes **all** category sets populate with non-zero counts at all three levels (CSA/ST1–ST4/TOT × sea/lea/sch). Regression-safe (FS226 fact view still 187). **Remaining for green:** build `Staging.vwMembership_StagingTables_052` mirroring the membership fact proc — expose `StudentIdentifierState` + org + `GradeLevel`/`Race`/`Sex` as EdFacts codes. Note the EdFacts-code columns live on the dim **tables** (`RDS.DimRaces/DimGradeLevels/DimK12Demographics`) while the `Map` columns for matching raw staging values live on the `vwDim*` views → the oracle needs a two-step join per dimension, plus race unduplication (`RDS.vwUnduplicatedRaceMap` + Hispanic override) and the Open/New school filter. The green FS226 view is the template.
