...
Surfaced `GradeLevelEdFactsCode` + `TitleIIndicatorEdFactsCode` on `RDS.vwTitleI_FactTable_134` (the latter needed `TitleI.TitleIIndicatorEdFactsCode` added to `debug.vwTitleI_FactTable` — sourced from `RDS.DimTitleIStatuses`). All FS134 sets populate non-zero (CSA/TOT × sea/lea; TOT-sea = 1556). **Regression-verified: FS222 e2e still GREEN (172) after the shared debug-view edit.** **Remaining for green:** build `Staging.vwTitleI_StagingTables_134`. Note the titleI staging base `debug.vwTitleI_StagingTables` does NOT expose `GradeLevel` (join `Staging.K12Enrollment` for it) and exposes the mapped `TitleIIndicator` (OutputCode, not the EdFacts code → two-step via `vwDimTitleIStatuses`→`DimTitleIStatuses`); population is Title-I participants (the base view already encodes the participation join + `sssrd.OutputCode IN ('TGELGBTGPROG','SWELIGTGPROG','SWELIGSWPROG')` school filter) intersected with the fact view's `SchoolOperationalStatus IN ('Open','New')`. Follow the FS052 recipe.
