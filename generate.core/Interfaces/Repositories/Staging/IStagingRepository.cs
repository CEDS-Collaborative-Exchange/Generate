...
namespace generate.core.Interfaces.Repositories.Staging
{
    public interface IStagingRepository
    {
        ...
        void CreateRangeSaveDetach<T>(IEnumerable<T> entities) where T : class;
        ...
        IEnumerable<T> GetDistinct<T>(Expression<Func<T, bool>> criteria, int skip = 0, int take = 50, params Expression<Func<T, object>>[] eagerLoad) where T : class;
