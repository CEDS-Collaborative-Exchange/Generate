...
{
    public class FactStudentAssessmentRepository : RDSRepository, IFactStudentAssessmentRepository
    {
        private readonly ILogger _logger;

        public FactStudentAssessmentRepository(
            ILogger<FactStudentAssessmentRepository> logger,
        ...
        public void Migrate_StudentAssessments(string factTypeCode)
        {

            int? oldTimeOut = _context.Database.GetCommandTimeout();
            _context.Database.SetCommandTimeout(11000);
            _context.Database.ExecuteSqlRaw("rds.Migrate_StudentAssessments @factTypeCode = {0}, @runAsTest = 0", factTypeCode);
            _context.Database.SetCommandTimeout(oldTimeOut);

        }


        public IEnumerable<ReportEDFactsK12StudentAssessment> Get_ReportData(string reportCode, string reportLevel, string reportYear, string categorySetCode, bool includeZeroCounts = false, bool includeFriendlyCaptions = false, bool obscureMissingCategoryCounts = false)
        {
