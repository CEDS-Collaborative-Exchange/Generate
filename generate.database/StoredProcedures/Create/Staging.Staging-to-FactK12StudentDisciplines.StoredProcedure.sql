...
-- Drop temp tables.  This allows for running the procedure as a script while debugging
		...
		IF OBJECT_ID('tempdb.dbo.#vwDisciplinesStatuses', 'U') IS NOT NULL DROP TABLE #vwDisciplinesStatuses
	...
	-- Creating temp tables to be used in the select statement joins 
		...
		SELECT * 
		INTO #vwDisciplinesStatuses 
		FROM RDS.vwDimDisciplineStatuses 
		WHERE SchoolYear = @SchoolYear
		
		CREATE INDEX IX_vwDimDisciplines ON #vwDisciplinesStatuses(SchoolYear, IdeaInterimRemovalMap, IdeaInterimRemovalReasonMap) INCLUDE (IdeaInterimRemovalCode, IdeaInterimRemovalReasonCode)
	...
	/*  Creating and load #Facts temp table */
		...
		CREATE TABLE #Facts (
			...
			, SEAId	int null
			, IEUId	int null
			, DataCollectionId int null --new
			, DisabilityStatusId int null -- new
			, DisciplinaryActionEndDateId  int null --new
			, DisciplinaryActionStartDateId int null
			, DisciplineStatusId int null --new
			, EconomicallyDisadvantagedStatusId int null --new
			, EnglishLearnerStatusId int null --new
			...
			, IncidentDateId int null --new
			, MigrantStatusId int null --new
			, MilitaryStatusId  int null --new
			, NOrDStatusId int null --new
			, PrimaryDisabilityTypeId int null --new
			, SecondaryDisabilityTypeId int null --new
			, TitleIStatusId int null --new
			, TitleIIIStatusId int null --new
			, DurationOfDisciplinaryAction int null --new
		...
		FROM Staging.Discipline sd 
			...
			-- disciplineId
			LEFT JOIN #vwDisciplinesStatuses rddisc
			...
			LEFT JOIN RDS.vwDimFirearmDisciplinesStatuses rdfds
					ON rsy.SchoolYear = rdis.SchoolYear				
