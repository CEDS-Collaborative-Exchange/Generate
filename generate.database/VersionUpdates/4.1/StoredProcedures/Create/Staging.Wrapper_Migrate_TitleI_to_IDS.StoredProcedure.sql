...
BEGIN
	...
	--Populate the IDS tables from Staging data
		...
		exec [Staging].[Migrate_StagingToIDS_Organization] @SchoolYear
			...
			--write out message to DataMigrationHistories
			insert into app.DataMigrationHistories
			(DataMigrationHistoryDate, DataMigrationTypeId, DataMigrationHistoryMessage) values	(getutcdate(), 1, 'IDS Migration Wrapper TitleI - Start OrganizationAddress')
		...
		exec [Staging].[Migrate_StagingToIDS_PersonStatus] @SchoolYear
			...
			--write out message to DataMigrationHistories
			insert into app.DataMigrationHistories
			(DataMigrationHistoryDate, DataMigrationTypeId, DataMigrationHistoryMessage) values	(getutcdate(), 1, 'IDS Migration Wrapper TitleI - Start ProgramParticipationSpecialEducation')
