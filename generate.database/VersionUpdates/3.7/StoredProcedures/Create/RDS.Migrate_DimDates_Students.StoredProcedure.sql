...
BEGIN
	...
	select 
		  @dateValue = DATEFROMPARTS(d.[Year], @cutOffMonth, @cutOffDay)
		, @yearStartDate = case when @isPriorYear = 0 then DATEFROMPARTS(d.[Year], @referencePeriodStartMonth, @referencePeriodStartDay)
						   else DATEFROMPARTS(d.[Year] - 1, @priorYearReferencePeriodStartMonth, @priorYearReferencePeriodStartDay) end
		, @yearEndDate = case when @isPriorYear = 0 then DATEFROMPARTS(d.[Year] + 1, @referencePeriodEndMonth, @referencePeriodEndDay)
						 else DATEFROMPARTS(d.[Year], @priorYearReferencePeriodEndMonth, @priorYearReferencePeriodEndDay) end
	from rds.DimDates d
	where d.DimDateId = @selectedDate

	select @dateValue = DATEADD(year, 1, @dateValue)
	where @dateValue < @yearStartDate

	select @dateValue = DATEADD(year, -1, @dateValue)
	where @dateValue > @yearEndDate
	...
	inner join ods.PersonIdentifier pi on s.StateStudentIdentifier = pi.Identifier
		AND s.RecordStartDateTime <= IIF(@useCutOffDate = 0, @yearEndDate, @dateValue)
		AND ((s.RecordEndDateTime >= IIF(@useCutOffDate = 0, @yearStartDate, @dateValue)) OR s.RecordEndDateTime IS NULL)
