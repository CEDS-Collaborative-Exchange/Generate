...
BEGIN
	...
	declare @CategorySetCode as varchar(50), @GenerateReportId as int, @CategorySetId as int, @LevelId as int, @fileSubmissionId int

	declare @reportTable as table (
		GenerateReportId int
	)

	IF NOT EXISTS(select 1 from rds.DimDates where SubmissionYear = @fromSY)
	BEGIN
		print 'Previous Year not found'
		return
	END

	IF NOT EXISTS(select 1 from rds.DimDates where SubmissionYear = @toSY)
	BEGIN
