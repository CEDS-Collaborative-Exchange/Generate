...
BEGIN
	...
	from
	(select s.DimStudentId, s.PersonId, s.DimCountDateId, lea.DimLeaID, leaDetail.OrganizationId as LeaOrganizationid
	from @studentDates s
	...
	and lea.DimLeaID > -1) leaLevel
	left outer join
	(select s.DimStudentId, s.PersonId, s.DimCountDateId, sch.DimSchoolId, o.OrganizationId as SchOrganizationId
	from @studentDates s
	inner join ods.OrganizationPersonRole r on r.PersonId = s.PersonId
		and r.RoleId = @k12StudentRoleId
