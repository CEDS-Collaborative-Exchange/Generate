	...
	BEGIN
		...
		BEGIN TRY
			UPDATE Staging.PersonStatus 
				SET EconomicDisadvantage_StatusStartDate = App.GetFiscalYearStartDate(@SchoolYear)
				, EconomicDisadvantage_StatusEndDate = NULL 
			WHERE EconomicDisadvantage_StatusStartDate IS NULL 
			AND EconomicDisadvantageStatus IS NOT NULL
			
			UPDATE Staging.PersonStatus 
				SET FosterCare_ProgramParticipationStartDate = App.GetFiscalYearStartDate(@SchoolYear)
				, FosterCare_ProgramParticipationEndDate = NULL 
			WHERE FosterCare_ProgramParticipationStartDate IS NULL 
			AND ProgramType_FosterCare IS NOT NULL
			
			UPDATE Staging.PersonStatus 
				SET Section504_ProgramParticipationStartDate = App.GetFiscalYearStartDate(@SchoolYear)
				, Section504_ProgramParticipationEndDate = NULL 
			WHERE Section504_ProgramParticipationStartDate IS NULL 
			AND ProgramType_Section504 IS NOT NULL
			
			UPDATE Staging.PersonStatus 
				SET Immigrant_ProgramParticipationStartDate = App.GetFiscalYearStartDate(@SchoolYear)
				, Immigrant_ProgramParticipationEndDate = NULL 
			WHERE Immigrant_ProgramParticipationStartDate IS NULL 
			AND ProgramType_Immigrant IS NOT NULL
			
			UPDATE Staging.PersonStatus 
				SET Homelessness_StatusStartDate = App.GetFiscalYearStartDate(@SchoolYear)
				, Homelessness_StatusEndDate = NULL 
			WHERE Homelessness_StatusStartDate IS NULL 
			AND HomelessnessStatus IS NOT NULL
			
			UPDATE Staging.PersonStatus 
				SET Migrant_StatusStartDate = App.GetFiscalYearStartDate(@SchoolYear)
				, Migrant_StatusEndDate = NULL 
			WHERE Migrant_StatusStartDate IS NULL 
			AND MigrantStatus IS NOT NULL
			
			UPDATE Staging.PersonStatus 
				SET EnglishLearner_StatusStartDate = App.GetFiscalYearStartDate(@SchoolYear)
				, EnglishLearner_StatusEndDate = NULL 
			WHERE EnglishLearner_StatusStartDate IS NULL 
			AND EnglishLearnerStatus IS NOT NULL
			
			UPDATE Staging.PersonStatus 
				SET PerkinsLEPStatus_StatusStartDate = App.GetFiscalYearStartDate(@SchoolYear)
				, PerkinsLEPStatus_StatusEndDate = NULL 
			WHERE PerkinsLEPStatus_StatusStartDate IS NULL 
			AND PerkinsLEPStatus IS NOT NULL
			
			UPDATE Staging.PersonStatus 
				SET IDEA_StatusStartDate = App.GetFiscalYearStartDate(@SchoolYear)
				, IDEA_StatusEndDate = NULL 
			WHERE IDEA_StatusStartDate IS NULL 
			AND IDEAIndicator IS NOT NULL
			
			UPDATE Staging.PersonStatus 
				SET MilitaryConnected_StatusStartDate = App.GetFiscalYearStartDate(@SchoolYear)
				, MilitaryConnected_StatusEndDate = NULL 
			WHERE MilitaryConnected_StatusStartDate IS NULL 
			
			AND MilitaryConnectedStudentIndicator IS NOT NULL
		END TRY
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_LEA', 'S05EC130' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationID_School', 'S05EC140' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_School', 'S05EC150' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationID_LEA_Program_Foster', 'S05EC160' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationID_School_Program_Foster', 'S05EC170' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationID_LEA_Program_Homeless', 'S05EC180' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationID_School_Program_Homeless', 'S05EC190' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationID_LEA_Program_Section504', 'S05EC200' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationID_School_Program_Section504', 'S05EC210' 
		END CATCH
		...
		BEGIN TRY
			...
			FROM Staging.PersonStatus ps
				...
				JOIN ODS.OrganizationProgramType orgpt 
					ON orgr.OrganizationId = orgpt.OrganizationId
			WHERE orgpt.RefProgramTypeId = App.GetProgramTypeId('04957')
		END TRY

		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationID_LEA_Program_Immigrant', 'S05EC220' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationID_School_Program_Immigrant', 'S05EC230' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonStatusId_EconomicDisadvantage', 'S05EC240' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonStatusId_EconomicDisadvantage', 'S05EC260' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonStatusId_Homeless', 'S05EC270' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonStatus', NULL, 'S05EC280' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonStatusId_Homeless', 'S05EC290' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonHomelessNightTimeResidenceId', 'S05EC300' 
		END CATCH
		...
		BEGIN TRY
			...
			FROM Staging.Person p
				JOIN Staging.PersonStatus ps 
					ON ps.Student_Identifier_State=p.Identifier
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonHomelessness', NULL, 'S05EC310' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonHomelessNightTimeResidenceId', 'S05EC320' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonStatusId_Migrant', 'S05EC340' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonStatus', NULL, 'S05EC350' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonStatusId_Migrant', 'S05EC360' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonStatusId_EnglishLearner', 'S05EC370' 
		END CATCH
		...
		BEGIN TRY
			...
			FROM Staging.PersonStatus ps
				LEFT JOIN [ODS].[PersonStatus] ps1 
			...
			WHERE ps.PersonStatusId_EnglishLearner IS NULL
			AND ps.EnglishLearnerStatus = 1
			AND ps.PersonId IS NOT NULL
			AND ps1.PersonStatusId IS NULL
		END TRY

		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonStatus', NULL, 'S05EC380' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonStatusId_EnglishLearner', 'S05EC390' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonStatus', NULL, 'S05EC400' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonLanguageId', 'S05EC410' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonLanguage', NULL, 'S05EC420' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonStatusId_IDEA', 'S05EC430' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonStatus', NULL, 'S05EC440' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonStatusId_IDEA', 'S05EC450' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonMilitaryId', 'S05EC460' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonMilitary', NULL, 'S05EC470' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'PersonMilitaryId', 'S05EC480' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_LEA_Program_Foster', 'S05EC490' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.OrganizationPersonRole', NULL, 'S05EC500' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_LEA_Program_Foster', 'S05EC510' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonProgramParticipation', NULL, 'S05EC520' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_School_Program_Foster', 'S05EC530' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.OrganizationPersonRole', NULL, 'S05EC540' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_School_Program_Foster', 'S05EC550' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonProgramParticipation', NULL, 'S05EC560' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationID_School_Program_Section504', 'S05EC570' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_LEA_Program_Section504', 'S05EC580' 
		END CATCH
		...
		BEGIN TRY	
			...
			AND ps.PersonId IS NOT NULL
			AND OrganizationId_LEA_Program_Section504 IS NOT NULL
		END TRY

		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.OrganizationPersonRole', NULL, 'S05EC590' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_LEA_Program_Section504', 'S05EC600' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonProgramParticipation', NULL, 'S05EC610' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_School_Program_Section504', 'S05EC620' 
		END CATCH
		...
		BEGIN TRY	
			...
			AND ps.ProgramType_Section504 = 1
			AND ps.PersonId IS NOT NULL
			AND OrganizationId_School_Program_Section504 IS NOT NULL
		END TRY

		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.OrganizationPersonRole', NULL, 'S05EC630' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_School_Program_Section504', 'S05EC640' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonProgramParticipation', NULL, 'S05EC650' 
		END CATCH
...
-------------------------------------------------------
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationID_School_Program_Immigrant', 'S05EC660' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_LEA_Program_Immigrant', 'S05EC670' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.OrganizationPersonRole', NULL, 'S05EC680' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_LEA_Program_Immigrant', 'S05EC690' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonProgramParticipation', NULL, 'S05EC700' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_School_Program_Immigrant', 'S05EC710' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.OrganizationPersonRole', NULL, 'S05EC720' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_School_Program_Immigrant', 'S05EC730' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonProgramParticipation', NULL, 'S05EC740' 
		END CATCH
...
-------------------------------------------------------
	...
	-----------------------------------------------------
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_LEA_Program_Homeless', 'S05EC750' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.OrganizationPersonRole', NULL, 'S05EC760' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_LEA_Program_Homeless', 'S05EC770' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonProgramParticipation', NULL, 'S05EC780' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_School_Program_Homeless', 'S05EC790' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.OrganizationPersonRole', NULL, 'S05EC800' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'Staging.PersonStatus', 'OrganizationPersonRoleID_School_Program_Homeless', 'S05EC810' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonProgramParticipation', NULL, 'S05EC820' 
		END CATCH
	...
	--============================================================================================================================
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.PersonDisability', NULL, 'S05EC830' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.K12StudentEnrollment', 'RefFoodServiceEligibilityId', 'S05EC840' 
		END CATCH
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.K12StudentEnrollment', 'NSLPDirectCertificationIndicator', 'S05EC850' 
		END CATCH
