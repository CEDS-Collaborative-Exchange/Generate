...
BEGIN
	...
	--Populate the RDS tables from ODS data
		...
		exec [rds].[Migrate_DimStudents]
			...
			--write out message to DataMigrationHistories
			insert into app.DataMigrationHistories
			(DataMigrationHistoryDate, DataMigrationTypeId, DataMigrationHistoryMessage) values	(getutcdate(), 2, 'RDS Migration Wrapper immigrant - Start Migrate_OrganizationCounts')
	...
	BEGIN CATCH
		insert into app.DataMigrationHistories
		(DataMigrationHistoryDate, DataMigrationTypeId, DataMigrationHistoryMessage) values	(getutcdate(), 2, 'RDS Migration Wrapper immigrant failed to run')
	END CATCH
