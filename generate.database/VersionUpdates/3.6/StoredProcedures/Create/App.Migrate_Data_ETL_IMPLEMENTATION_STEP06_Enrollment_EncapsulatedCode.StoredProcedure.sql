...
---Additional Items to add in the future - exit/withdraw reason--
		...
		BEGIN TRY
			...
			FROM Staging.Enrollment e
				JOIN ODS.OrganizationIdentifier orgid 
					ON e.LEA_Identifier_State = orgid.Identifier
		...
		BEGIN TRY
			...
			WHEN NOT MATCHED 
				AND ISNULL(SOURCE.OrganizationPersonRoleId_School,'') <> ''
				AND ISNULL(SOURCE.OrganizationPersonRoleId_Lea,'') <> '' 
				THEN INSERT (
					OrganizationPersonRoleId
		...
		--Update the Staging table with the OPR relationship value
		BEGIN TRY 
			UPDATE enr
		...
		--End-date any records that require it
		BEGIN TRY
			UPDATE r 
			SET r.RecordEndDateTime = s.RecordStartDateTime - 1
			FROM ods.OrganizationPersonRoleRelationship r				
				JOIN (
					...
					ON  r.OrganizationPersonRoleId = s.OrganizationPersonRoleId
		...
		BEGIN CATCH 
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'OrganizationPersonRoleRelationship', 'RecordEndDate', 'S06EC250'
		END CATCH
		...
		BEGIN TRY
			INSERT INTO [ODS].[RoleAttendance] (
					...
					, [AttendanceRate])
			SELECT 
				opr.OrganizationPersonRoleID											[OrganizationPersonRoleId] 
				,CASE
					WHEN ISNULL(en.NumberOfSchoolDays, 0) > 0 
					THEN en.NumberOfSchoolDays - ISNULL(en.[NumberOfDaysAbsent], 0)
				END																		[NumberOfDaysInAttendance]
				, en.[NumberOfDaysAbsent]												[NumberOfDaysAbsent]
				,CASE
					WHEN ISNULL(en.NumberOfSchoolDays, 0) > 0 
					THEN CAST( (en.NumberOfSchoolDays - ISNULL(en.NumberOfDaysAbsent, 0.0))/en.NumberOfSchoolDays AS  Decimal(5,4))
				END																		[AttendanceRate]
			FROM ods.OrganizationPersonRole opr
				INNER JOIN staging.Enrollment en 
					ON en.OrganizationID_School = opr.OrganizationId 
					AND en.PersonId = opr.PersonId
				LEFT JOIN [ODS].[RoleAttendance] oa 
					ON oa.OrganizationPersonRoleId = opr.OrganizationPersonRoleId
		...
		BEGIN TRY
			...
			SELECT @PsEnrolled = [RefPsEnrollmentActionId] FROM [ODS].[RefPsEnrollmentAction] WHERE [Code] = 'Enrolled'
			SELECT @psUnenrolled = [RefPsEnrollmentActionId] FROM [ODS].[RefPsEnrollmentAction] WHERE [Code] = 'NotEnrolled'
			...
			FROM [ODS].[K12StudentAcademicRecord] sar
				JOIN [ODS].OrganizationPersonRole opr 
					ON opr.OrganizationPersonRoleID = sar.OrganizationPersonRoleID
				JOIN staging.Enrollment en 
					ON en.OrganizationID_School = opr.OrganizationId 
					AND en.PersonId = opr.PersonId
		...
		BEGIN CATCH
			EXEC App.Migrate_Data_Validation_Logging @eStoredProc, 'ODS.StudentAcademicRecord', NULL, 'S06EC330' 
		END CATCH
