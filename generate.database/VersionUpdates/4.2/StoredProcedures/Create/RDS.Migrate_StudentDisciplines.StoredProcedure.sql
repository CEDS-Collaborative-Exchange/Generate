...
----Migrate_DimFirearms
	...
	SELECT DISTINCT 
		...
		dis.IncidentId,
		ISNULL(CAST(rds.Get_Age(stu.Birthdate, DATEFROMPARTS(CASE WHEN @cutOffMonth >= 7 THEN @submissionYear - 1 ELSE @submissionYear END, @cutOffMonth, @cutOffDay)) AS VARCHAR(50)), 'MISSING') AS AgeCode,
		ISNULL(grade.EntryGradeLevelCode,'MISSING'),
		ISNULL(grade.DimEntryGradeLevelId, -1),
...
--		ISNULL(demo.LepStatusCode, 'MISSING'),
	...
	JOIN RDS.DimK12Students stu
		ON s.DimK12StudentId = stu.DimK12StudentId
	...
	LEFT JOIN #demoQuery demo 
		...
		AND sch.DimK12SchoolId = demo.DimK12SchoolId 
		AND sch.DimLeaId = demo.DimLeaId
	...
	LEFT JOIN @ideaQuery idea 
		...
		AND idea.DimK12SchoolId = sch.DimK12SchoolId 
	...
	BEGIN
		...
		SELECT 
			...
			ISNULL(q.DimEntryGradeLevelId, -1) AS DimGradeLevelId,
			ISNULL(q.DimK12DemographicId, -1) AS DimDemographicId,
			ISNULL(q.DimIdeaStatusId, -1) AS DimIdeaStatusId,
			ISNULL(programStatus.DimProgramStatusId, -1) AS DimProgramStatusId,
			ISNULL(q.DimDisciplineId, -1) AS DimDisciplineId,
			ISNULL(firearms.DimFirearmsId,-1) AS DimFirearmsId,
			ISNULL(q.DimFirearmDisciplineId,-1) AS DimFirearmsDisciplineId,
			--sum( 
			CASE WHEN ISNULL(q.DimDisciplineId, -1) <> -1 THEN 1 ELSE 0 END AS DisciplineCount,
			CASE WHEN ISNULL(q.DimDisciplineId, -1) <> -1 THEN sum(q.DisciplineDuration) ELSE 0 END AS DisciplineDuration,
		...
		LEFT JOIN rds.DimFirearms firearms 
			ON firearms.FirearmTypeCode = replace(q.FirearmsCode,'RiflesShotguns','RIFLESHOTGUN')
		...
		q.DimSchoolYearId,
		ISNULL(a.DimAgeId, -1),
		ISNULL(q.DimEntryGradeLevelId, -1),
		ISNULL(q.DimK12DemographicId, -1),
		ISNULL(q.DimIdeaStatusId, -1),
		ISNULL(q.DimDisciplineId, -1),
		isnull(q.IncidentId, -1),
		ISNULL(programStatus.DimProgramStatusId, -1),
		ISNULL(firearms.DimFirearmsId, -1),
		ISNULL(q.DimFirearmDisciplineId,-1),
