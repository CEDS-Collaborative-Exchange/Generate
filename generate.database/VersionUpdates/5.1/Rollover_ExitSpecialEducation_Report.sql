if not exists(select 1 from app.CategorySets where GenerateReportId = 31 and SubmissionYear = 2022)
BEGIN

	exec App.Rollover_Previous_Year_Metadata 'ExitSpecialEducation', 2021, 2022
...
begin try
	...
	-------------Initializing variables for each category set
					...
					IF(@reportTypeId=@generateReportTypeId)
						BEGIN
						SET @viewDefinitiationStart='{"showColumnTotals":0,"showRowTotals":0,"defaultFilterType":3,'	
						END
					ELSE
						BEGIN
	...
	-----------------------assigining odd number categories to columns
					...
					else if(@updatedFactFieldName='staffFTE')
					begin
						SET @fields=@fields+ '{"binding":"' + @updatedFactFieldName + '","header":"FTE","dataType":2,"aggregate":1,"showAs":0,"descending":false,"format":"n2","isContentHtml":false}'	
						set @valueFields=',"valueFields":{"items":["FTE"]}'
					end
					else
					begin				
