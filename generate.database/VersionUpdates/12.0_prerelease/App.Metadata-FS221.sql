...
BEGIN
	...
	VALUES(2,3,1,@reportCode,'N or D Exit Outcomes – LEA',@reportCode,1,1,0,0,0,0,1)
END

UPDATE app.GenerateReports SET FactTableId = @factTableId, ReportTypeAbbreviation = 'NDEXITLEA' where ReportCode = @reportCode
...
IF NOT EXISTS(SELECT 1 FROM app.TableTypes where TableTypeAbbrv = 'NDEXITOUTLEA')
BEGIN
	INSERT INTO [App].[TableTypes](TableTypeAbbrv, TableTypeName, EdFactsTableTypeId)
    VALUES('NDEXITOUTLEA', 'N or D Exit Outcomes – LEA', -1)
END

SELECT @tableTypeId = TableTypeId FROM app.TableTypes where TableTypeAbbrv = 'NDEXITOUTLEA'
...
If NOT EXISTS(Select 1 from App.FileSubmissions where FileSubmissionDescription = 'LEA ND EXIT OUTCOMES LEA' and GenerateReportId = @generateReportId
...
BEGIN
	...
	values ('LEA ND EXIT OUTCOMES LEA', @GenerateReportId, 2, '2024')
 
END

SELECT @fileSubmissionId = FileSubmissionId FROM app.FileSubmissions where FileSubmissionDescription = 'LEA ND EXIT OUTCOMES LEA'
