...
begin transaction	
	...
	declare	@seaStateIdentifer varchar(50),	@stateANSICode varchar(10),	@LeaTypeCode varchar(50), @LeaTypeDescription varchar(100),	@LeaTypeEdFactsCode varchar(50),
	@LeaTypeId int,	@MailingAddressStreet varchar(40),	@MailingAddressCity as varchar(30),	@MailingAddressState varchar(50), @MailingAddressPostalCode varchar(20),	
	@OutOfStateIndicator bit,	@PhysicalAddressStreet  varchar(40), @PhysicalAddressCity  varchar(30) , @PhysicalAddressState  varchar(50), @PhysicalAddressPostalCode  varchar(20),
	@charterAuthorizerType varchar(50),	@charterManagerType varchar(50), @SchoolTypeCode   varchar(50),	@SchoolTypeDescription  varchar(100), @SchoolTypeEdFactsCode  varchar(50),
	@SchoolTypeId int, @Telephone  varchar(24),	@Website  varchar(300),@effectiveDate datetime,	@leaName varchar(1000), @LeaNcesIdentifier varchar(50), @leaOrganizationId int, 
	@leaOrganizationIds int, @LeaStateIdentifiers varchar(50), @LeaStateIdentifier varchar(50),	@PriorLeaStateIdentifier varchar(50), @SupervisoryIdNum varchar(3),
	@ReportedFederally  bit = 1, @dimSeaId int = 0,	@dimLeaId int = 0,	@dimDirectoryId int = 0, @dimPersonnelId int = 0
	...
	if not exists (select 1 from RDS.DimPersonnel)
	begin
	...
	begin
		...
		select distinct o.Name, o.OrganizationId, i.Identifier, i.Identifier, s.Code, rsCode.StateName,	leaType.Code as LeaTypeCode, leaType.Description as LeaTypeDescription,
			CASE leaType.Code 
			...
			END, -- EdFact
			leaType.RefLeaTypeId, mailingAddress.StreetNumberAndName, mailingAddress.City, mailingAddress.StateCode, mailingAddress.PostalCode, 
			'0', physicalAddress.StreetNumberAndName, physicalAddress.City,	physicalAddress.StateCode,	physicalAddress.PostalCode,					
		...
		left join (select ol.OrganizationId, ol.LocationId, RefOrganizationLocationTypeId, StreetNumberAndName, City, refState.Code as StateCode, PostalCode  from ODS.OrganizationLocation ol
			...
			left join rds.DimSeas sea on sea.SeaStateIdentifier = i.Identifier
		where o.RefOrganizationTypeId = @seaOrgTypeId 
		AND o.RecordEndDateTime IS NULL AND sea.DimSeaId IS NULL
		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				--insert into DimSea 
				INSERT INTO [RDS].[DimSeas] ([SeaName],[SeaOrganizationId],[SeaStateIdentifier],[StateANSICode],[StateCode],[StateName]
											,[MailingAddressStreet],[MailingAddressCity],[MailingAddressState],[MailingAddressPostalCode]
											,[PhysicalAddressStreet],[PhysicalAddressCity],[PhysicalAddressState],[PhysicalAddressPostalCode],[Telephone],[Website])
				VALUES(@seaName, @SeaOrganizationId, @seaStateIdentifer, @stateANSICode, @stateCode, @stateName,@MailingAddressStreet,@MailingAddressCity
											,@MailingAddressState,@MailingAddressPostalCode,@PhysicalAddressStreet,@PhysicalAddressCity,@PhysicalAddressState
				...
				INSERT INTO [RDS].[DimPersonnel]([BirthDate],[FirstName],[LastName],[MiddleName],[PersonnelPersonId],[StatePersonnelIdentifier],[Email],[Telephone],[Title],PersonnelRole)
				select p.Birthdate,	p.FirstName, p.LastName, p.MiddleName, p.PersonId, 	Identifier, EmailAddress,TelephoneNumber,PositionTitle,	@personnelRole
				...
				left join rds.DimPersonnel personnnel on personnnel.StatePersonnelIdentifier = i.Identifier
				where r.OrganizationId = @SeaOrganizationId and personnnel.DimPersonnelId IS NULL
	...
	--DimLeasSeed
		...
		CASE WHEN t.Code = 'LEANotFederal' THEN 0 ELSE 1 END AS ReportedFederally		
							from  ods.OrganizationDetail o 								
								...
								left outer join rds.DimLeas leas on leas.LeaStateIdentifier = i1.Identifier
								where 
								--o.RefOrganizationTypeId = @leaOrgTypeId 
								--and 
								(t.Code = 'LEA' OR t.Code = 'LEANotFederal')
								AND o.RecordEndDateTime IS NULL AND leas.DimLeaID IS NULL
		...
		BEGIN
			...
			INSERT INTO [RDS].[DimLeas]([LeaName],[LeaNcesIdentifier],[LeaOrganizationId],[LeaStateIdentifier],
			[SeaName],[SeaOrganizationId],[SeaStateIdentifier],[StateANSICode],[StateCode],[StateName],[SupervisoryUnionIdentificationNumber],EffectiveDate,PriorLeaStateIdentifier,ReportedFederally,
									   [LeaTypeCode],[LeaTypeDescription],[LeaTypeEdFactsCode],[LeaTypeId],[MailingAddressStreet],[MailingAddressCity],[MailingAddressState]
										,[MailingAddressPostalCode],[OutOfStateIndicator],[PhysicalAddressStreet],[PhysicalAddressCity],[PhysicalAddressState],[PhysicalAddressPostalCode]
										,[Telephone],[Website]) 
		    values(@leaName, @LeaNcesIdentifier, @leaOrganizationId, @LeaStateIdentifier,@seaName, @seaOrganizationId, @seaIdentifier,@seaIdentifier, @stateCode, @stateName, 
				   @SupervisoryIdNum,@effectiveDate,@PriorLeaStateIdentifier,@ReportedFederally, @LeaTypeCode,@LeaTypeDescription,@LeaTypeEdFactsCode,@LeaTypeId,
										@MailingAddressStreet,@MailingAddressCity,@MailingAddressState,@MailingAddressPostalCode,@OutOfStateIndicator,@PhysicalAddressStreet,	
										@PhysicalAddressCity,@PhysicalAddressState,	@PhysicalAddressPostalCode,	@Telephone,	@Website)

		   SET @dimLeaId = SCOPE_IDENTITY()


		INSERT INTO [RDS].[BridgeLeaGradeLevels]
           ([DimLeaId]
           ,[DimGradeLevelId])
		SELECT distinct @dimLeaId, grades.DimGradeLevelId 
			FROM ods.OrganizationDetail o
	...
	declare @IsCharterSchool as bit
	...
	inner join ods.OrganizationIdentifier i on o.OrganizationId =i.OrganizationId
								...
								and i.RefOrganizationIdentifierTypeId = @schoolIdentifierTypeId
	INNER JOIN ODS.K12School K12 ON K12.OrganizationId=O.OrganizationId
	INNER JOIN ODS.RefOrganizationType t ON t.RefOrganizationTypeId = o.RefOrganizationTypeId
	LEFT OUTER JOIN RDS.DimSchools sch on sch.SchoolStateIdentifier = i.Identifier
	where (t.Code = 'K12School' OR t.Code = 'K12SchoolNotFederal')
	AND o.RecordEndDateTime IS NULL AND sch.DimSchoolId IS NULL
	...
	BEGIN
		...
		begin
			...
			WHILE @@FETCH_STATUS = 0
			BEGIN
				INSERT INTO [RDS].[DimCharterSchoolApproverAgency]([Name],[OrganizationId],[StateIdentifier],[StateCode],[StateANSICode] 
				,[State] ,[OrganizationType],[IsApproverAgency],[SeaOrganizationId],[LeaTypeCode],[LeaTypeDescription],[LeaTypeEdFactsCode],[LeaTypeId],[MailingAddressStreet],[MailingAddressCity],[MailingAddressState],[MailingAddressPostalCode],[OutOfStateIndicator],					  [PhysicalAddressStreet],[PhysicalAddressCity],[PhysicalAddressState],[PhysicalAddressPostalCode],[Telephone],[Website],[SchOrganizationId])
				values (@leaName, @leaOrganizationIds, @LeaStateIdentifiers, @stateCode, @seaIdentifier, @stateName, @charterManagerType,'No', @seaOrganizationId,	@LeaTypeCode,@LeaTypeDescription,@LeaTypeEdFactsCode,@LeaTypeId, @MailingAddressStreet,@MailingAddressCity,@MailingAddressState,@MailingAddressPostalCode,
						 1, @PhysicalAddressStreet, @PhysicalAddressCity, @PhysicalAddressState, @PhysicalAddressPostalCode,	@Telephone,	@Website, @schoolOrganizationId)
			...
			FETCH NEXT FROM CharterSchoolManager_Cursor INTO @leaName, @leaOrganizationIds, @LeaStateIdentifiers,
										@LeaTypeCode, @LeaTypeDescription, @LeaTypeEdFactsCode, @LeaTypeId,
										@MailingAddressStreet, @MailingAddressCity, @MailingAddressState, @MailingAddressPostalCode,
										@PhysicalAddressStreet, @PhysicalAddressCity, @PhysicalAddressState, @PhysicalAddressPostalCode,											@Telephone, @Website, @charterManagerType
			END
			...
			BEGIN
				...
				INSERT INTO [RDS].[DimCharterSchoolApproverAgency]([Name],[OrganizationId],[StateIdentifier],[StateCode],[StateANSICode] ,
				[State] ,[OrganizationType],[IsApproverAgency],[SeaOrganizationId],[LeaTypeCode],[LeaTypeDescription],[LeaTypeEdFactsCode],
				[LeaTypeId],[MailingAddressStreet],[MailingAddressCity],[MailingAddressState],[MailingAddressPostalCode],OutOfStateIndicator,
				[PhysicalAddressStreet],[PhysicalAddressCity],[PhysicalAddressState],[PhysicalAddressPostalCode],[Telephone],[Website],
				[SchOrganizationId])
				values (@leaName, @leaOrganizationIds, @LeaStateIdentifiers, @stateCode, @seaIdentifier, @stateName, @charterAuthorizerType,
				 'Yes', @seaOrganizationId,	@LeaTypeCode,@LeaTypeDescription,@LeaTypeEdFactsCode,@LeaTypeId,
				  @MailingAddressStreet,@MailingAddressCity,@MailingAddressState,@MailingAddressPostalCode,
				  1, @PhysicalAddressStreet, @PhysicalAddressCity, @PhysicalAddressState, @PhysicalAddressPostalCode,	@Telephone,	@Website,
		...
		insert into rds.DimSchools(StateCode, StateName, StateANSICode, SeaOrganizationId, SeaName,	SeaStateIdentifier,	LeaOrganizationId,LeaNcesIdentifier,LeaStateIdentifier, LeaName,
		  			               SchoolOrganizationId, SchoolNcesIdentifier,	SchoolStateIdentifier, SchoolName,	EffectiveDate,	PriorLeaStateIdentifier, PriorSchoolStateIdentifier,
								   CharterSchoolIndicator, CharterContractApprovalDate,	CharterContractRenewalDate,	CharterSchoolContractIdNumber, ReportedFederally, [LeaTypeCode],[LeaTypeDescription],[LeaTypeEdFactsCode],[LeaTypeId],
								   [MailingAddressStreet],[MailingAddressCity],[MailingAddressState]
									,[MailingAddressPostalCode],[OutOfStateIndicator],[PhysicalAddressStreet],[PhysicalAddressCity],[PhysicalAddressState],[PhysicalAddressPostalCode]
									,[SchoolTypeCode] ,[SchoolTypeDescription] ,[SchoolTypeEdFactsCode]	,[SchoolTypeId]	,[Telephone],[Website])
		select distinct	@stateCode,	@stateName,	@seaIdentifier,	@seaOrganizationId,	@seaName, @seaIdentifier, @leaOrganizationId, @leaNcesIdentifier, @leaStateIdentifier,
						@leaOrganizationName, @schoolOrganizationId, @schoolNcesIdentifier,	@schoolStateIdentifier,	@schoolName, @effectiveDate, @PriorLeaStateIdentifier,
						@priorSchoolStateIdentifier, @charterSchoolIdentifier,	@charterSchoolContractApprovalDate,	@charterSchoolContractRenewalDate, @charterSchoolContractIdNumber,
						@ReportedFederally,	leaType.Code, leaType.Description,
						CASE leaType.Code 
							WHEN 'RegularNotInSupervisoryUnion' then 1
							WHEN 'RegularInSupervisoryUnion' then 2
							WHEN 'SupervisoryUnion' then 3
						...
						schoolType.Code, schoolType.Description,
						CASE schoolType.Code 
								WHEN 'Regular' then 1
								WHEN 'Special' then 2
								WHEN 'CareerAndTechnical' then 3
								...
								WHEN 'Reportable' then 5
						END,
						schoolType.RefSchoolTypeId, phone.TelephoneNumber,	website.Website
		from ods.OrganizationDetail o 								
		left join ods.OrganizationWebsite website on o.OrganizationId = website.OrganizationId
		left join ods.OrganizationTelephone phone on o.OrganizationId = phone.OrganizationId
		left join  
		...
		INSERT INTO [RDS].[BridgeSchoolGradeLevels]([DimSchoolId],[DimGradeLevelId])
		SELECT distinct @dimSchoolId, grades.DimGradeLevelId 
		FROM ods.OrganizationDetail o
		inner join ods.K12School sch on o.OrganizationId = sch.OrganizationId
		inner join ods.K12SchoolGradeOffered g on g.K12SchoolId = sch.K12SchoolId
		inner join ods.RefGradeLevel l on l.RefGradeLevelId = g.RefGradeLevelId
		...
		where DimCountDateId in (select d.DimDateId from rds.DimDates d
		...
		declare SEA_Cursor cursor for 						
		select DimSeaId from rds.DimSeas where DimSeaId <> -1
		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				inner join ods.[Role] r on p.PersonnelRole = r.Name
				where r.roleId = @CSSORoleId
				
			
				INSERT INTO [RDS].[FactOrganizationCounts]([DimCountDateId],[DimDirectoryStatusId],
				[DimOrganizationStatusId],[DimFactTypeId],[DimLeaId],[DimPersonnelId]
					,[DimSchoolId],[DimSchoolStatusId],[DimSeaId],[DimTitle1StatusId],[OrganizationCount],
					[DimCharterSchoolApproverAgencyId],[DimCharterSchoolManagerOrganizationId]
					,[DimCharterSchoolSecondaryApproverAgencyId],[DimCharterSchoolUpdatedManagerOrganizationId]
					,[DimSchoolStateStatusId])
				SELECT d.DimDateId as DimCountDateId, -1, -1,	@factTypeId, -1 as 'DimLeaId', 
				isnull( @dimPersonnelId, -1) as 'DimPersonnelId',-1 as 'DimSchoolId',
					-1 as 'DimSchoolStatusId', @dimSeaId, -1 as 'DimTitle1StatusId', 1 as OrganizationCount, 
		...
		declare LEA_Cursor cursor for 						
		select DimLeaId, LeaOrganizationId from rds.DimLeas where DimLeaId <> -1
		...
		BEGIN
			...
			delete from rds.BridgeLeaDate where DimLeaId = @dimLeaId

			INSERT INTO [RDS].[BridgeLeaDate]([DimLeaId],[DimDateId])
			SELECT @dimLeaId, d.DimDateId
			from rds.DimDates d
			inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId 
			inner join rds.DimDataMigrationTypes b on b.DimDataMigrationTypeId=dd.DataMigrationTypeId 
			where d.DimDateId <> -1 and dd.IsSelected=1 and DataMigrationTypeCode= 'rds'
		...
		exec rds.Migrate_DimDates_Leas @dimLeaId
			...
			exec [RDS].[Migrate_DimDirectoryStatuses_Lea] @LeaDateQuery
			...
			SELECT DimCountDateId,leadate.dimLeaID, leaFund.ParentalInvolvementReservationFunds,
			...
			inner join	(select l.*, @leaOrganizationId as organizationId from  @LeaDateQuery l) as leaDate 
			...
			select s.DimCountDateId,
			 ...
			 From @LeaDateQuery s
			left outer join @leaDirectoryStatusQuery dirStatus on s.DimLeaId = dirStatus.DimLeaId and s.DimCountDateId = dirStatus.DimCountDateId
			left outer join @leaFederalFundQuery fund on s.DimLeaId = fund.DimLeaId and s.DimCountDateId = fund.DimCountDateId 
			left outer join @leaOrganizationStatusQuery dimOrganizationStatus on s.DimLeaId = dimOrganizationStatus.DimLeaId 
		...
		select DimSchoolId, SchoolOrganizationId  from rds.DimSchools where DimSchoolId <> -1

		open SCH_Cursor
		FETCH NEXT FROM SCH_Cursor INTO @dimSchoolId, @schoolOrganizationId							
		...
		BEGIN
			...
			delete from rds.BridgeSchoolDate where DimSchoolId = @dimSchoolId

			INSERT INTO rds.BridgeSchoolDate([DimSchoolId],[DimDateId])
			SELECT @dimSchoolId, d.DimDateId
			from rds.DimDates d
			inner join rds.DimDateDataMigrationTypes dd on dd.DimDateId=d.DimDateId 
			inner join rds.DimDataMigrationTypes b on b.DimDataMigrationTypeId=dd.DataMigrationTypeId 
			where d.DimDateId <> -1 and dd.IsSelected=1 and DataMigrationTypeCode= 'rds'
			...
			exec rds.Migrate_DimDates_Schools @dimSchoolId, @schoolOrganizationId
			...
			exec [RDS].[Migrate_DimDirectoryStatuses_School] @SchoolDateQuery
			...
			inner join ods.RefSchoolImprovementFunds refSis on schImprv.RefSchoolImprovementFundsId = refSis.RefSchoolImprovementFundsId 
			and refSis.Code = 'YES'
			inner join	(select l.*, @schoolOrganizationId as organizationId from  @SchoolDateQuery l) as schDate 
			...
			select @IsCharterSchool = CharterSchoolIndicator from rds.DimSchools where SchoolOrganizationId = @schoolOrganizationId

			if @IsCharterSchool = 1
			begin
				
				select @count = count(DimCharterSchoolApproverAgencyId) from rds.DimCharterSchoolApproverAgency 
				where SchOrganizationId = @schoolOrganizationId and IsApproverAgency = 'No'
				...
				select @count = count(DimCharterSchoolApproverAgencyId) from rds.DimCharterSchoolApproverAgency 
				where SchOrganizationId = @schoolOrganizationId and IsApproverAgency = 'Yes'
			...
			select s.DimCountDateId,
				...
				ISNULL(dirStatus.OperationalStatusCode ,'MISSING'),
				ISNULL(dirStatus.UpdatedOperationalStatusCode ,'MISSING'),
				ISNULL(dirStatus.McKinneyVentoSubgrantRecipient ,'MISSING'),
			...
			left outer join @SchImprovementFundQuery schImprFund on s.DimSchoolId = schImprFund.DimSchoolId 
			and s.DimCountDateId = schImprFund.DimCountDateId
			left outer join @organizationStatusSchoolQuery organizationStatus on s.DimSchoolId = organizationStatus.DimSchoolId 
