CREATE PROCEDURE [RDS].[Migrate_DimMigrant]
 ...
 BEGIN
	...
	inner join ods.OrganizationPersonRole opr on opr.PersonId = s.PersonId
		...
		and opr.EntryDate <= s.SubmissionYearEndDate and (opr.ExitDate >=  s.SubmissionYearStartDate or opr.ExitDate is null)
	left join ods.OrganizationPersonRole r2 on r2.PersonId = s.PersonId
		and r2.EntryDate <= s.SubmissionYearEndDate and (r2.ExitDate >=  s.SubmissionYearStartDate or opr.ExitDate is null)
