...
BEGIN
	...
	if @reportCode in ('c175','c178','c179') and @reportLevel <> 'sea' and LEN(ISNULL(@categorySetCode,'')) < 1
	begin
	...
	begin
		set @sqlInsertDefinition = @sqlInsertDefinition + 'PersonnelCount [int] NOT NULL,'
	...
	begin
		...
		begin
			...
			' 
				...
				SELECT @performanceCountSql = [RDS].[Get_CountSQL] (@reportCode, @reportLevel, @reportYear, @catSetCode, 'performanceLevels',0, 0,@tableTypeAbbrvs,@totalIndicators)
	...
	begin		
		...
		begin
			...
			BEGIN
				...
				SELECT @zeroCountSql = [RDS].[Get_CountSQL] (@reportCode, @reportLevel, @reportYear, @catSetCode, 'zero',@includeOrganizationSQL, 1,@tableTypeAbbrvs, @totalIndicators)
			...
			BEGIN
					...
					set @sql = @sql + 'delete a from @reportData a
						inner join 
							(select OrganizationId, CategorySetCode, sum(StudentCount) as totalStudentCount from @reportData group by organizationID, CategorySetCode having sum(StudentCount) < 1 and CategorySetCode <> ''TOT''	) b
							on a.CategorySetCode = b.CategorySetCode and a.OrganizationId = b.OrganizationId'


					set @sql = @sql + ' delete a from @reportData a 
					inner join 
						(select OrganizationId, GRADELEVEL, CategorySetCode, sum(StudentCount) as totalStudentCount from @reportData group by organizationID, GRADELEVEL, CategorySetCode having sum(StudentCount) < 1 and 
						CategorySetCode in (''CSA'', ''ST1'', ''ST2'' )) b
						on a.CategorySetCode = b.CategorySetCode and a.OrganizationId = b.OrganizationId  and a.GRADELEVEL = b.GRADELEVEL
						'
			END
		...
		begin
			set @includeOrganizationSQL = 1
			SELECT @zeroCountSql = [RDS].[Get_CountSQL] (@reportCode, @reportLevel, @reportYear, @categorySetCode, 'zero',@includeOrganizationSQL, 0,@tableTypeAbbrvs, @totalIndicators)
	...
	if(LEN(ISNULL(@categorySetCode,'')) < 1) AND @reportLevel <> 'sea' AND @reportCode = 'c002'
    begin
         set @includeOrganizationSQL = 1
         SELECT @zeroCountSql = [RDS].[Get_CountSQL] (@reportCode, @reportLevel, @reportYear, 'TOT', 'zero',@includeOrganizationSQL, 1,@tableTypeAbbrvs, @totalIndicators)
