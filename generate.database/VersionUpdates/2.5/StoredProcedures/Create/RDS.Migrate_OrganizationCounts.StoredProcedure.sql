...
declare @factTable as varchar(50)
	...
	-- SEA
			...
			IF not exists (select 1 from RDS.DimSeas where DimSeaId <> -1)
			 BEGIN
						...
						schoolType.Code as 'EdFacts' ,-- todo 
					...
					-- insert the corresponding Directory info 
							...
							INSERT INTO [RDS].[BridgeDirectoryDate]
									   ([DimDirectoryId]
									   ,[DimDateId])
							SELECT @dimDirectoryId, DimDateId
	...
	BEGIN
		...
		isnull(priorLea.Identifier,'') -- PriorLeaStateIdentifier				
							from  ods.OrganizationDetail o 								
								...
								where o.RefOrganizationTypeId = @leaOrgTypeId 
								AND o.RecordEndDateTime IS NULL
		...
		FETCH NEXT FROM LEA_Cursor INTO @leaName, @LeaNcesIdentifier, @leaOrganizationId, @LeaStateIdentifier,@SupervisoryIdNum,
											...
											@PhysicalAddressStreet, @PhysicalAddressCity, @PhysicalAddressState, @PhysicalAddressPostalCode,
											@SchoolTypeCode, @SchoolTypeDescription, @SchoolTypeEdFactsCode, @SchoolTypeId,
											@Telephone, @Website, @effectiveDate , @PriorLeaStateIdentifier						
		...
		exec rds.Migrate_DimDates_Directories @dimDirectoryId, @dimLeaId
	...
	leaFund.ParentalInvolvementReservationFunds,
			case when federalProgramCode ='84.010' Then FederalProgramsFundingAllocation
			else 0 end as 'FederalProgramsFundingAllocation'
	...
	select s.DimCountDateId,
		...
		round(FederalProgramsFundingAllocation,0),
		dirStatus.McKinneyVentoSubgrantRecipient
							...
							INSERT INTO [RDS].[FactOrganizationCounts]
							  ...
							  isnull(q.TitleiPartaAllocations,0),
							   ...
							   from #leaQueryOutput q
								...
								on case when dirStatus.CharterLeaStatusCode = 'MISSING' THEN 'MISSING'
										 when (dirStatus.CharterLeaStatusCode = 'CHRTIDEAESEA' or dirStatus.CharterLeaStatusCode ='CHRTIDEA'or dirStatus.CharterLeaStatusCode='CHRTIDEA' or dirStatus.CharterLeaStatusCode='CHRTESEA') THEN 'YES'
										 when (dirStatus.CharterLeaStatusCode ='NOTCHR' or dirStatus.CharterLeaStatusCode = 'NA') THEN 'NO'
										 ELSE dirStatus.CharterLeaStatusCode end  = q.CharterLeaStatusCode
	...
	DECLARE school_cursor CURSOR FOR 
		select distinct O.OrganizationId, Name, K12.CharterSchoolIndicator 
			from
			ods.OrganizationDetail o
			INNER JOIN ODS.K12School K12 ON K12.OrganizationId=O.OrganizationId
			where o.RefOrganizationTypeId = @schoolOrgTypeId
			AND o.RecordEndDateTime IS NULL
		
			OPEN school_cursor
			FETCH NEXT FROM school_cursor INTO @schoolOrganizationId, @schoolName, @IsCharterSchool
		...
		FETCH NEXT FROM CharterSchoolManager_Cursor INTO @leaName, @leaOrganizationIds, @LeaStateIdentifiers,
					...
					INSERT INTO [RDS].[DimCharterSchoolApproverAgency]
										([Name],[OrganizationId],[StateIdentifier],[StateCode],[StateANSICode] ,[State] ,[OrganizationType]	,[IsApproverAgency]	,[SeaOrganizationId]) 
		   
										values (@leaName, @leaOrganizationIds, @LeaStateIdentifiers, @stateCode, @seaIdentifier, @stateName, @charterManagerType,'No', @seaOrganizationId)
								...
								END
		   						...
		   						INSERT INTO [RDS].[DimDirectories]( [LeaTypeCode],[LeaTypeDescription] ,[LeaTypeEdFactsCode] ,[LeaTypeId] ,[MailingAddressStreet]  ,[MailingAddressCity] ,[MailingAddressState],[MailingAddressPostalCode]
...
---------------------------------------------
		...
		exec rds.Migrate_DimDates_Directories @dimDirectoryId, @dimSchoolId
	...
	----insert into FactDirectoryCounts
	 INSERT INTO [RDS].[FactOrganizationCounts]
							   ([DimCountDateId]
							   ,[DimDirectoryId]
							   ,[DimDirectoryStatusId]
	...
	begin
		...
		begin
			...
			exec [RDS].[Create_OrganizationReportData]	@reportCode = 'c170', @runAsTest = @runAsTest
				...
				insert into app.DataMigrationHistories
			(DataMigrationHistoryDate, DataMigrationTypeId, DataMigrationHistoryMessage) 
			values	(getutcdate(), @dataMigrationTypeId, 'c205 - Progress Achieving English Language Proficiency')
