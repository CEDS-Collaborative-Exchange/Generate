...
BEGIN
	...
	insert into @submissionYears
		...
		select distinct SubmissionYear from app.CategorySets Where GenerateReportId in 
			(select GenerateReportId from app.GenerateReports where ReportCode = 'cohortgraduationrate') 
	end
	else
	begin
		insert into @submissionYears
			...
			begin
				...
				begin
					...
					begin
						...
						SeaName,
						SeaStateIdentifier,
						case when @reportLevel = 'lea' then LeaOrganizationId
							 when @reportLevel = 'sch' then SchoolOrganizationId
							 else SeaOrganizationId end as OrganizationId,
						case when @reportLevel = 'lea' then LeaNcesIdentifier
							 when @reportLevel = 'sch' then SchoolNcesIdentifier
							 else SeaStateIdentifier end as OrganizationNcesIdentifier,
						case when @reportLevel = 'lea' then LeaStateIdentifier
							 when @reportLevel = 'sch' then SchoolStateIdentifier
							 else SeaStateIdentifier end as OrganizationStateIdentifier,
						case when @reportLevel = 'lea' then LeaName
							 when @reportLevel = 'sch' then SchoolName
							 else SeaName end as OrganizationName,
						demo.SexDescription,
					...
					insert into @cohortStudentCounts
						...
						SUM([5]), 
						SUM([6])
						FROM (select f.DimStudentId,StateANSICode,StateCode,StateName,
						SeaOrganizationId,
						SeaName,
						SeaStateIdentifier,
						case when @reportLevel = 'lea' then LeaOrganizationId
						...
						UNION
						select distinct f.DimStudentId,StateANSICode,StateCode,StateName,
						SeaOrganizationId,
						SeaName,
						SeaStateIdentifier,
						case when @reportLevel = 'lea' then LeaOrganizationId
