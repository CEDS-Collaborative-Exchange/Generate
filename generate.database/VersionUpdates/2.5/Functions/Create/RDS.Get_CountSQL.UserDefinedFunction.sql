...
BEGIN
	...
	-- Get Toggle Values
		...
		Select @toggleGradCompltrResponse =
								  COALESCE(@toggleGradCompltrResponse + ', ' +
								  ...
								  case when ResponseValue = 'Regular diploma that indicates a student meets or exceeds the requirements of a regular diploma.' THEN '''REGDIP'''
									   When ResponseValue ='Other high school completion credentials for meeting criteria other than the requirements for a regular diploma(i.e. certificate of completion, certificate of attendance).' THEN 'OTHCOM' 
									   end ,
			...
			Set @toggleGradCompltrResponse = ISNULL(@toggleGradCompltrResponse,'''REGDIP'', ''OTHCOM''')
				...
				select @toggleCteDiploma = COALESCE(@toggleCteDiploma + ', ''', '''') + case when ResponseValue = 'Regular secondary school diploma' THEN 'REGDIP'
	...
	BEGIN
		...
		begin
			...
			begin
				set @sqlCategoryReturnField = '
				case 
					when sum(isnull(fact.DisciplineDuration, 0)) < 1.5 then ''LTOREQ1''
	...
	having 1 = 1'
			...
			begin
				...
				inner join RDS.BridgeStudentRaces b on fact.' + @personField + ' = b.' + @personField + '
				inner join rds.DimRaces CAT_' + @reportField + ' on b.DimRaceId = CAT_' + @reportField + '.DimRaceId 
...
--
		...
		set @sqlCountJoins = @sqlCountJoins + '
			inner join (
				...
				select distinct fact.DimStudentId
					...
					left outer join ods.RefGradeLevel gl on enroll.RefExitGradeLevel = gl.RefGradeLevelId
					where fact.DimCountDateId = @dimDateId
					and fact.DimFactTypeId = @dimFactTypeId
					and fact.dimschoolid <> -1
		...
		begin
			...
			else if(@reportCode in ('c005','c088','c144'))
			begin
				set @sql = @sql + '
		...
		begin
		-- Counts are stored as discipline/assessment counts in fact table, but reports should report on student counts
			set @sumOperation = 'sum( case when ' + @factField + ' > 0 then 1 else 0 end )'
		end
		else if @reportCode in ('c005','c006','c088','c144','c009')
		begin
	...
	begin
		...
		begin
			...
			set @sql = @sql + '
			delete a from #performanceData_' + @categorySetCode + ' a
			inner join ( select  AssessmentTypeCode, Grade, PerformanceLevels
