...
--print '	delete from app.TableTypes where EdFactsTableTypeId not in (select Id from @recordIds)'
--print ''
...
begin try
	...
	(
	  Id int
		...
		If Not exists (Select 1 from App.TableTypes where EdFactsTableTypeId = 238)
		BEGIN
			INSERT INTO App.TableTypes
			([EdFactsTableTypeId], [TableTypeAbbrv], [TableTypeName])
			VALUES
			(238, 'MEPPRIORITYSRV', 'MEP Students Priority For Services Table' )
		...
		ELSE
		BEGIN
			UPDATE App.TableTypes Set [TableTypeAbbrv] = 'MEPPRIORITYSRV', [TableTypeName] = 'MEP Students Priority For Services Table' where EdFactsTableTypeId = 238
		END
