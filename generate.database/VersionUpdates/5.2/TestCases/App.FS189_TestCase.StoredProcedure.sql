CREATE OR ALTER PROCEDURE [App].[FS189_TestCase]	
	@SchoolYear SMALLINT
AS
BEGIN
...
BEGIN TRY
	...
	DECLARE @UnitTestName VARCHAR(100) = 'FS189_UnitTestCase'
	DECLARE @AssessmentAcademicSubject VARCHAR(10) = '00562' 
	DECLARE @StoredProcedureName VARCHAR(100) = 'FS189_TestCase'
	DECLARE @TestScope VARCHAR(1000) = 'FS189'
	DECLARE @ReportCode VARCHAR(20) = 'C189' 
	DECLARE @SubjectAbbrv VARCHAR(20) = 'SCIENCE'
	...
	CREATE NONCLUSTERED INDEX IX_asr ON #asr ([Student_Identifier_State],[LEA_Identifier_State],[School_Identifier_State],[GradeLevelWhenAssessed],AssessmentTitle,AssessmentAcademicSubject,AssessmentPurpose,AssessmentRegistrationParticipationIndicator,AssessmentRegistrationReasonNotCompleting,AssessmentTypeAdministeredToChildrenWithDisabilities,SchoolYear,AssessmentAdministrationStartDate,AssessmentAdministrationFinishDate)
	...
	CREATE NONCLUSTERED INDEX IX_a ON #a (AssessmentTitle,AssessmentAcademicSubject,AssessmentPurpose,AssessmentTypeAdministeredToChildrenWithDisabilities)
	...
	SELECT 
		...
		a.[AssessmentPurpose],
		[ParticipationStatus] = CASE asr.AssessmentRegistrationParticipationIndicator
								WHEN 1
								    THEN CASE asr.AssessmentTypeAdministeredToChildrenWithDisabilities
										  WHEN 'REGASSWOACC'
											 THEN 'REGPARTWOACC'
										  WHEN 'REGASSWACC'
											 THEN 'REGPARTWACC'
										  WHEN 'ALTASSGRADELVL'
											 THEN 'ALTPARTGRADELVL'
										  WHEN 'ALTASSMODACH'
											 THEN 'ALTPARTMODACH'
										  WHEN 'ALTASSALTACH'
											 THEN 'ALTPARTALTACH'
								...
								WHEN 0
								    THEN CASE 
										  WHEN asr.AssessmentRegistrationReasonNotCompleting = 'Medical'
											 THEN 'MEDEXEMPT'
										  WHEN asr.AssessmentRegistrationReasonNotCompleting = 'PARTELP'
											 THEN 'PARTELP'
										  WHEN asr.AssessmentRegistrationReasonNotCompleting IS NULL
											 THEN 'MISSING'
										  WHEN asr.AssessmentRegistrationReasonNotCompleting IS NOT NULL AND asr.AssessmentRegistrationReasonNotCompleting NOT IN ('Medical','PARTELP') 
											 THEN 'NPART'
									   END
							END,
...
----------------------------------------------------------------------------------------------------
		...
		SELECT 
			...
			,'CSA ' + UPPER(sar.ReportLevel) + ' Match All - Participation Status: ' + s.ParticipationStatus + '; Grade Level: ' + s.GradeLevelWhenAssessed  
		...
		FROM #TC1 s
		JOIN #csa sar 
			ON s.ParticipationStatus = sar.ParticipationStatus
		...
		SELECT 
			...
			,'CSB ' + UPPER(sar.ReportLevel) + ' Match All - Participation Status: ' + s.ParticipationStatus + '; Grade Level: ' + s.GradeLevelWhenAssessed  
		...
		SELECT 
			...
			,'CSC ' + UPPER(sar.ReportLevel) + ' Match All - Participation Status: ' + s.ParticipationStatus + '; Grade Level: ' + s.GradeLevelWhenAssessed 
		...
		SELECT 
			...
			,'CSD ' + UPPER(sar.ReportLevel) + ' Match All - Participation Status: ' + s.ParticipationStatus + '; Grade Level: ' + s.GradeLevelWhenAssessed 
		...
		SELECT 
			...
			,'CSE ' + UPPER(sar.ReportLevel) + ' Match All - Participation Status: ' + s.ParticipationStatus + '; Grade Level: ' + s.GradeLevelWhenAssessed 
		...
		SELECT 
			...
			,'CSF ' + UPPER(sar.ReportLevel) + ' Match All - Participation Status: ' + s.ParticipationStatus + '; Grade Level: ' + s.GradeLevelWhenAssessed
		...
		SELECT 
			...
			,'CSG ' + UPPER(sar.ReportLevel) + ' Match All - Participation Status: ' + s.ParticipationStatus + '; Grade Level: ' + s.GradeLevelWhenAssessed
		...
		SELECT 
			...
			,'CSH ' + UPPER(sar.ReportLevel) + ' Match All - Participation Status: ' + s.ParticipationStatus + '; Grade Level: ' + s.GradeLevelWhenAssessed
		...
		SELECT 
			...
			,'CSI ' + UPPER(sar.ReportLevel) + ' Match All - Participation Status: ' + s.ParticipationStatus + '; Grade Level: ' + s.GradeLevelWhenAssessed
		...
		SELECT 
			...
			,'ST1 ' + UPPER(sar.ReportLevel) + ' Match All - Participation Status: ' + s.ParticipationStatus + '; Grade Level: ' + s.GradeLevelWhenAssessed
			,s.AssessmentCount
			,sar.AssessmentCount
			,CASE WHEN s.AssessmentCount = ISNULL(sar.AssessmentCount, -1) THEN 1 ELSE 0 END
			,GETDATE()
