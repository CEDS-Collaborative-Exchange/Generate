...
BEGIN
	...
	begin
		if(@reportCode in ('c002','c089', 'yeartoyearenvironmentcount', 'yeartoyearchildcount','yeartoyearexitcount','studentssummary'))
		begin
	...
	BEGIN
		...
		BEGIN
			...
			set @reportCount = 0
			SET @sqlAlreadyRun = N'SELECT @countOUT = count(*)
								  from RDS.' + @factReportTable + '
									where ReportCode = @reportCode 
									and ReportYear = @reportYear 
									and ReportLevel = @reportLevel
									and CategorySetCode = @categorySetCode'
			
			if @reportCode='c150' OR @reportCode='c151'
                begin
                    set @sqlAlreadyRun = @sqlAlreadyRun + '
                        and TableTypeAbbrv = ''' +  @TableTypeAbbrv + '''
                    '
                end

			SET @sqlAlreadyRunParm = N'@reportCode varchar(50), @reportYear varchar(50), @reportLevel varchar(5), @categorySetCode varchar(50), @countOUT int OUTPUT'
			EXECUTE sp_executesql @sqlAlreadyRun, @sqlAlreadyRunParm, @reportCode = @reportCode, @reportYear = @reportYear, @reportLevel = @reportLevel, @categorySetCode = @categorySetCode, @countOUT=@reportCount OUTPUT

			print '-- Starting Row Count: ' + convert(varchar(20), @reportCount)

			if @reportCount = 0 or @runAsTest = 1 OR @reportCode='c204'
			begin
				...
				select @sql = [RDS].[Get_CountSQL] (@reportCode, @reportLevel, @reportYear, @categorySetCode, 'actual', 1, 0, @tableTypeAbbrvs, @totalIndicators)
				...
				set @reportCount = 0
				EXECUTE sp_executesql @sqlAlreadyRun, @sqlAlreadyRunParm, @reportCode = @reportCode, @reportYear = @reportYear, @reportLevel = @reportLevel, @categorySetCode = @categorySetCode, @countOUT=@reportCount OUTPUT
				print '-- Ending Row Count: ' + convert(varchar(20), @reportCount)
			...
			begin
				...
				--print '@sqlAlreadyRun: ' + @sqlAlreadyRun
