CREATE PROCEDURE [RDS].[Get_YearToYearRemovalCountReportData]
...
BEGIN
	...
	DECLARE @DeclareSQL nvarchar(MAX), @InsertSQL nvarchar(MAX), @SelectSQL nvarchar(MAX), @GroupBySQL nvarchar(MAX), @count as int, @categoryType nvarchar(max), @countType nvarchar (max)
	...
	begin
		if @categorySetCode not in ('removaltype','raceethnic', 'disabilitytype', 'gender','lepstatus', 'age')
		begin
		SET @categoryType=@categoryType + ', REMOVALTYPE'
		end
		set @catSetTemp= @catSetTemp + ' DisciplineCount int NULL '
		set @countType = @countType + 'DisciplineCount' 
	end
	else if(@reportCode='yeartoyearexitcount')
	begin
		if @categorySetCode not in ('exitonly','raceethnicity', 'disabilitytype', 'gender','lepstatus', 'age')
		begin
			set @categoryType=', BASISOFEXIT'
		end
		set @catSetTemp = @catSetTemp + ' StudentCount int NULL '
		set @countType = @countType + 'StudentCount' 
	end
	...
	BEGIN  
		...
		IF(@rowField not in ('REMOVALTYPE','BASISOFEXIT') and @count > 1)
		BEGIN
