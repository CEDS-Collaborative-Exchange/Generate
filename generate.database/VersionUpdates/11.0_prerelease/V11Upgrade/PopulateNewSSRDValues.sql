if not exists (select top 1 * from Staging.SourceSystemReferenceData where SchoolYear = 2023 and TableName = 'RefIdeaDisabilityType')
	begin
		insert into staging.SourceSystemReferenceData
		select 2023, 'RefIdeaDisabilityType', NULL, Code, Code
		from dbo.RefIdeaDisabilityType
	end
